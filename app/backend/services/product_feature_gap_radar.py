    ...
    def _feature_gaps(self) -> tuple[ProductFeatureGap, ...]:
        return (
            ...
            ProductFeatureGap(
                ...
                current_state="Generated document storage, generated_documents CRUD quota guard, template governance evidence, export readiness, delivery package manifest checks, version diff checklist, and a best-effort frontend report-quota consumption attempt after successful generation exist, but clause variables, renderer enforcement, and quota coverage for direct case/deep-review generation paths are incomplete.",
                ...
                next_actions=(
                    ...
                    "Extend the CRUD quota guard to direct case and deep-review document generation paths.",
                ),
            ),
            ProductFeatureGap(
                id="contract-review",
            ...
            ProductFeatureGap(
                ...
                module="billing",
                current_state="Entitlement checks, local quota policy, privacy-safe quota persistence and migration plans, quota counter repository, entitlement/quota binding facade, authenticated billing usage routes, report preflight, generated_documents CRUD quota guard, typed frontend API helper, a global workspace quota badge, a best-effort document-generation quota consumption attempt, and local-only payment/invoice/plan-change reconciliation policy evidence exist, but product packaging, real payment provider settlement verification, webhook signature handling, and quota coverage for direct case/deep-review generation paths are incomplete.",
                ...
                next_actions=(
                    "Extend quota enforcement to direct case and deep-review document generation paths.",
            ...
            ProductFeatureGap(
                ...
                module="model_ops",
                current_state="Model routing, budgets, compatibility checks, and a laptop-safe legal document benchmark suite exist, but cost observability, result archives, and cheap-first guardrails still need product hardening.",
            ...
            ProductFeatureGap(
                ...
                current_state="Knowledge, RAG audits, failure fixtures, source-freshness policy, source ingestion metadata contracts, durable index plan, metadata-only index repository, indexed retrieval-plan binding, metadata-only legal RAG routes, typed frontend API helper, route discovery smoke, a case research metadata-only RAG panel with localStorage summary cache, Copy context, Clear cache, and selected-source request metadata propagation into case AI prompts, generation plans, and citation maps exist, but full report citation validation against selected sources is incomplete.",
                ...
                next_actions=(
                    "Validate generated report citations against selected source ids and stale-source policy.",
