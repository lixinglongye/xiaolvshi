    ...
    def _feature_gaps(self) -> tuple[ProductFeatureGap, ...]:
        return (
            ProductFeatureGap(
                ...
                current_state="Case primitives, dashboard payload contract, notification policy, persistence plan, validated state repository, runtime binding facade, authenticated runtime routes, typed frontend API helper, route discovery smoke, and a case overview runtime state/event panel exist, but full editing flows and live risk-state updates are not complete.",
                ...
                next_actions=(
                    "Bind persisted material, fact, and task edits to privacy-safe workbench state events.",
                    "Attach repository events to task notification and escalation policy updates.",
            ...
            ProductFeatureGap(
                ...
                module="billing",
                current_state="Entitlement checks, local quota policy, privacy-safe quota persistence and migration plans, quota counter repository, entitlement/quota binding facade, authenticated billing usage routes, typed frontend API helper, and a global workspace quota badge exist, but product packaging, invoice states, payment provider integration, and deep-review consumption wiring are incomplete.",
                ...
                next_actions=(
                    "Record deep-review consumption through the entitlement quota binding service.",
            ...
            ProductFeatureGap(
                ...
                current_state="Knowledge, RAG audits, failure fixtures, source-freshness policy, source ingestion metadata contracts, durable index plan, metadata-only index repository, indexed retrieval-plan binding, metadata-only legal RAG routes, typed frontend API helper, route discovery smoke, and a case research metadata-only RAG panel exist, but selected-source IDs still need to feed case AI prompts and report citations.",
