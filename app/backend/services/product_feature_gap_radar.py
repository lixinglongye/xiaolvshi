    ...
    def _feature_gaps(self) -> tuple[ProductFeatureGap, ...]:
        return (
            ProductFeatureGap(
                ...
                current_state="Case primitives, dashboard payload contract, notification policy, persistence plan, validated state repository, runtime binding facade, authenticated runtime routes, typed frontend API helper, route discovery smoke, a case overview runtime state/event panel, and privacy-safe material/evidence/fact/task edit event binding exist, but collaborative editing, escalation wiring, and live risk-state updates are not complete.",
                ...
                next_actions=(
                    "Replay workbench edit events into task notification and escalation policy updates.",
            ...
            ProductFeatureGap(
                ...
                current_state="Generated document storage, template governance evidence, export readiness, delivery package manifest checks, version diff checklist, and a best-effort frontend report-quota consumption attempt after successful generation exist, but clause variables, renderer enforcement, and server-side quota enforcement are incomplete.",
                ...
                next_actions=(
                    ...
                    "Move best-effort quota consumption into server-side deep-review full quota enforcement.",
                ),
            ),
            ProductFeatureGap(
                id="contract-review",
            ...
            ProductFeatureGap(
                ...
                module="billing",
                current_state="Entitlement checks, local quota policy, privacy-safe quota persistence and migration plans, quota counter repository, entitlement/quota binding facade, authenticated billing usage routes, report preflight, typed frontend API helper, a global workspace quota badge, and a best-effort document-generation quota consumption attempt exist, but product packaging, invoice states, payment provider reconciliation, and server-side deep-review full quota enforcement are incomplete.",
                ...
                next_actions=(
                    "Move best-effort document-generation consumption into server-side deep-review full quota enforcement.",
                    "Add payment provider state reconciliation without storing payment secrets.",
                    "Expose invoice and plan-change states without storing payment secrets.",
                ),
            ),
            ProductFeatureGap(
                id="feedback-loop",
            ...
            ProductFeatureGap(
                ...
                current_state="Knowledge, RAG audits, failure fixtures, source-freshness policy, source ingestion metadata contracts, durable index plan, metadata-only index repository, indexed retrieval-plan binding, metadata-only legal RAG routes, typed frontend API helper, route discovery smoke, and a case research metadata-only RAG panel with localStorage summary cache, Copy context, and Clear cache exist, but selected-source IDs still need to feed case AI prompts and report citations.",
                ...
                next_actions=(
                    "Carry selected source ids into case AI prompts and report citation checks.",
