    ...
    def _feature_gaps(self) -> tuple[ProductFeatureGap, ...]:
        return (
            ProductFeatureGap(
                ...
                current_state="Case primitives, dashboard payload contract, notification policy, persistence plan, validated state repository, runtime binding facade, authenticated runtime routes, typed frontend API helper, route discovery smoke, a case overview runtime state/event panel, privacy-safe material/evidence/fact/task edit event binding, and task runtime notification policy summaries exist, but collaborative editing, notification dispatch, escalation persistence, and live risk-state updates are not complete.",
                ...
                next_actions=(
                    ...
                    "Connect runtime event deltas to live risk-state and evidence graph refreshes.",
                ),
            ),
            ProductFeatureGap(
            ...
            ProductFeatureGap(
                ...
                module="model_ops",
                current_state="Model routing, budgets, compatibility checks, Gemini/NewAPI cheap-first policy, task-level model selector audits, deterministic selector replay evidence, metadata-only cheap-first calibration in AIHub and ModelOps, privacy-safe route telemetry repository aggregates, persisted route telemetry operations summary checks, deep-review document quota blocking before AI drafting, a laptop-safe legal document benchmark suite, a LegalBench/LexGLUE/COLIEE research registry, a maintenance UI for the metadata-only registry, and a public adoption/research bridge UI evidence exist, but live cost dashboards, result archives, and cheap-first guardrails still need product hardening.",
                ...
                next_actions=(
                    ...
                    "Use cheap-first calibration before promoting legal review routes beyond Flash-Lite precheck or balanced Flash.",
