    ...
    def _entries(self) -> list[LedgerEntry]:
        return [
            ...
            LedgerEntry(
                ...
                size="medium",
                status="shipped",
                impact="Maps canonical, models/, google/, yibu/, and nested OpenAI-compatible Gemini model aliases to catalog ids, cheap-first eligibility, premium exceptions, and explicit-only review states without gateway calls or raw payload storage.",
                evidence_paths=(
            ...
            LedgerEntry(
                ...
                size="medium",
                status="shipped",
                impact="Turns sanitized observed Gemini-like gateway model ids into manual ModelProfile candidate rows, required metadata checks, and explicit-only default-promotion boundaries without editing model_catalog.py or calling gateways.",
                evidence_paths=(
            ...
            LedgerEntry(
                ...
                size="medium",
                status="shipped",
                impact="Adds metadata-only observed Gemini model intake queue evidence so OpenAI-compatible gateway /models or manually observed Gemini-like model ids can be normalized and marked with known or unknown status, price, lifecycle, cost tier, cheap-first eligibility, and default-promotion block/review/ready state before they enter default candidates, without NewAPI/Gemini/OpenAI/Google/gateway/network calls, real environment writes, raw prompts, payloads, model outputs, or credentials.",
                evidence_paths=(
                    "app/backend/services/release_readiness.py",
                    "app/backend/services/continuous_update_ledger.py",
                    "app/backend/services/maintenance_evidence.py",
