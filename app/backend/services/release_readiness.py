    ...
    def _checks(self) -> list[ReleaseCheck]:
        return [
            ...
            ReleaseCheck(
                ...
                validation_command=(
                    "python -m pytest tests/test_model_ops_gemini_official_cheap_first_source_review.py "
                    "tests/test_model_catalog_source_audit.py tests/test_model_ops_readiness.py "
                ...
                manual_note=(
                    "This is metadata-only official Gemini cheap-first source review evidence for Flash-Lite, "
                    "Flash, and Pro pricing ratios, high-frequency default alignment, source freshness, and "
                    "default-promotion blockers; it does not call NewAPI, Gemini, OpenAI, Google, gateways, "
                    "app AI endpoints, models, or the network, write configuration, change defaults, shift "
                    "traffic, claim pricing accuracy, claim automatic default changes, or return API keys, "
                    "Authorization headers, request bodies, response bodies, prompts, raw payloads, raw legal "
                    "text, model outputs, emails, or credentials."
                ),
            ),
            ReleaseCheck(
            ...
            ReleaseCheck(
                ...
                validation_command="python -m pytest tests/test_model_gateway_connection_profile.py tests/test_model_gateway_health_plan.py tests/test_aihub_runtime_routing.py tests/test_model_ops_readiness.py tests/test_frontend_ui_regression_gate.py -q",
                manual_note="This is metadata-only OpenAI-compatible gateway connection profile evidence for NewAPI/Gemini base URL shape, /v1 runtime normalization, key-presence placeholders, and cheap-first role readiness; it does not call NewAPI, Gemini, OpenAI, Google, gateways, app AI endpoints, or the network, does not write configuration or shift traffic, and does not include API keys, Authorization headers, request bodies, response bodies, prompts, raw payloads, legal text, model outputs, gateway responses, credentials, emails, or user identifiers.",
            ),
            ReleaseCheck(
                ...
                validation_command="python -m pytest tests/test_model_gateway_runtime_configuration.py tests/test_model_gateway_connection_profile.py tests/test_model_gateway_health_plan.py tests/test_model_ops_readiness.py tests/test_release_readiness.py tests/test_frontend_ui_regression_gate.py -q",
                manual_note="This is metadata-only runtime gateway configuration evidence for APP_AI_BASE_URL normalization, APP_AI_KEY placeholder use, cheap-first Gemini role defaults, and safe probe ordering before live gateway use; it does not call NewAPI, Gemini, OpenAI, Google, yibuapi, gateways, app AI endpoints, models, or the network, does not write environment files, configuration, default routes, or traffic, and does not include API keys, Authorization headers, request bodies, response bodies, prompts, raw payloads, legal text, model outputs, gateway responses, credentials, emails, or user identifiers.",
            ),
            ReleaseCheck(
                ...
                validation_command="python -m pytest tests/test_model_ops_newapi_channel_bootstrap.py tests/test_model_gateway_connection_profile.py tests/test_model_gateway_runtime_configuration.py tests/test_model_ops_observed_gemini_premium_exception_review.py tests/test_release_readiness.py tests/test_continuous_update_ledger.py tests/test_maintenance_evidence.py tests/test_frontend_ui_regression_gate.py -q && cd ../frontend && npm run typecheck && npm run ui:regression",
                manual_note="This is metadata-only NewAPI channel cheap-first bootstrap evidence for yibuapi/OpenAI-compatible channel URL normalization, APP_AI_KEY placeholder setup, cheap-first Gemini defaults, sanitized observed model intake, coverage-gap review, explicit-only premium exception review, and maintenance UI review; it does not call NewAPI, Gemini, OpenAI, Google, yibuapi, gateways, app AI endpoints, models, or the network, does not write environment files, configuration, default routes, or traffic, and does not include API keys, Authorization headers, request bodies, response bodies, prompts, raw payloads, legal text, model outputs, gateway responses, credentials, emails, or user identifiers.",
            ),
            ReleaseCheck(
                ...
                validation_command="python -m pytest tests/test_modelops_observed_gateway_model_fit_matrix.py tests/test_model_ops_readiness.py tests/test_release_readiness.py tests/test_continuous_update_ledger.py tests/test_maintenance_evidence.py tests/test_frontend_ui_regression_gate.py -q",
                manual_note="This is metadata-only observed gateway model fit matrix evidence for sanitized NewAPI/Gemini/OpenAI-compatible model-list ids, cheap-first task coverage, lowest-cost observed candidates, and review-only boundaries for Pro, preview, image, unknown, external, and unpriced models; it does not call NewAPI, Gemini, OpenAI, Google, gateways, app AI endpoints, or the network, does not write configuration, change defaults, shift traffic, validate an account inventory, or include API keys, Authorization headers, request bodies, response bodies, prompts, raw payloads, legal text, model outputs, gateway responses, credentials, emails, or user identifiers.",
            ),
            ReleaseCheck(
                ...
                validation_command="python -m pytest tests/test_model_ops_runtime_explicit_model_fit_gate.py tests/test_model_runtime_router.py tests/test_aihub_runtime_routing.py tests/test_model_ops_readiness.py tests/test_release_readiness.py tests/test_continuous_update_ledger.py tests/test_maintenance_evidence.py tests/test_frontend_ui_regression_gate.py -q",
                manual_note="This is metadata-only runtime explicit model fit evidence for sanitized task/model scenarios, unknown gateway guard review, reviewed gateway pass-through exceptions, explicit over-budget exceptions, local downgrade visibility, cheap-first enforcement signals, observed gateway fit linkage, and privacy/claim boundaries; it reflects the local runtime router behavior that guards unknown and non-stable explicit models by default, does not call NewAPI, Gemini, OpenAI, Google, gateways, app AI endpoints, models, or the network, does not write configuration, change defaults, shift traffic, validate an account inventory, or include API keys, Authorization headers, request bodies, response bodies, headers, messages, prompts, raw payloads, legal text, model outputs, gateway responses, credentials, emails, or user identifiers.",
            ),
            ReleaseCheck(
            ...
            ReleaseCheck(
                ...
                validation_command="python -m pytest tests/test_model_gateway_probe_runbook_gate.py tests/test_model_gateway_health_plan.py tests/test_model_gateway_probe_evaluation.py tests/test_model_gateway_runtime_configuration.py tests/test_model_ops_newapi_channel_bootstrap.py tests/test_model_ops_readiness.py tests/test_frontend_ui_regression_gate.py -q && cd ../frontend && npm run typecheck && npm run ui:regression",
                manual_note="This is metadata-only model gateway probe runbook evidence for the ordered NewAPI/Gemini rollout sequence: runtime/channel normalization, secret-boundary verification, list-models first, cheap JSON probe, optional image smoke, legal fixture smoke, and maintainer default-change review. It does not call NewAPI, Gemini, OpenAI, Google, yibuapi, gateways, app AI endpoints, models, or the network, does not write environment files, configuration, default routes, or traffic, does not change defaults or shift traffic, and does not include API keys, Authorization headers, request bodies, response bodies, raw probe payloads, prompts, raw legal text, model outputs, gateway responses, credentials, emails, or user identifiers.",
            ),
            ReleaseCheck(
            ...
            ReleaseCheck(
                ...
                title="ModelOps cheap-first cascade research gate",
                category="model_ops",
                required=True,
                owner="engineering",
                evidence_paths=(
                ...
                validation_command="python -m pytest tests/test_model_ops_cheap_first_cascade_research_gate.py tests/test_model_route_quality_budget.py tests/test_model_ops_cheap_first_escalation_budget.py tests/test_model_failure_upgrade_budget.py tests/test_model_ops_readiness.py tests/test_release_readiness.py tests/test_continuous_update_ledger.py -q",
                manual_note="This is metadata-only cheap-first cascade research evidence tying FrugalGPT-style cascade justification and official Gemini Flash-Lite cheap-start positioning to local route quality, escalation budget, failure upgrade, calibration, and user-need handoff gates; it does not call NewAPI, Gemini, OpenAI, Google, gateways, model endpoints, public datasets, or the network, write configuration, change default routes, shift traffic, claim production accuracy, or return prompts, raw legal text, raw model output, payloads, headers, identifiers, or credentials.",
            ),
            ReleaseCheck(
            ...
            ReleaseCheck(
                ...
                validation_command="python -m pytest tests/test_model_ops_user_need_release_bridge.py tests/test_user_need_implementation_priority_queue.py tests/test_user_need_gemini_route_coverage.py tests/test_model_ops_cheap_first_release_decision.py tests/test_model_ops_readiness.py tests/test_release_readiness.py tests/test_frontend_ui_regression_gate.py -q && cd ../frontend && npm run typecheck && npm run ui:regression",
                manual_note="This is metadata-only user-need release bridge evidence for cheap-first default reviews; it joins user-need implementation planning with Gemini route coverage and public benchmark license review, but does not call NewAPI, Gemini, OpenAI, Google, gateways, or public datasets, write configuration, shift traffic, claim public benchmark scores, or return raw legal text, benchmark samples, prompts, model outputs, payloads, emails, identifiers, or credentials.",
            ),
            ReleaseCheck(
                ...
                validation_command="python -m pytest tests/test_model_ops_user_need_cheap_first_handoff.py tests/test_model_ops_user_need_release_bridge.py tests/test_user_need_implementation_priority_queue.py tests/test_user_need_gemini_route_coverage.py tests/test_model_ops_readiness.py tests/test_release_readiness.py tests/test_continuous_update_ledger.py tests/test_frontend_ui_regression_gate.py -q && cd ../frontend && npm run typecheck && npm run ui:regression",
                manual_note="This is metadata-only user-need cheap-first handoff evidence for maintainer review of cheap-first default changes; it aggregates user-need benchmark coverage, implementation queue rows, Gemini route coverage, and the ModelOps user-need release bridge through /api/v1/aihub/models/user-need-cheap-first-handoff and /api/v1/maintenance/user-needs/cheap-first-evidence-handoff, but does not call NewAPI, Gemini, OpenAI, Google, gateways, app AI endpoints, public datasets, or the network, write configuration, change default routes, shift traffic, claim public benchmark scores, or return raw legal text, benchmark samples, fixture snippets, prompts, model outputs, payloads, headers, emails, identifiers, or credentials.",
            ),
            ReleaseCheck(
                ...
                validation_command="python -m pytest tests/test_modelops_legal_benchmark_default_promotion_bridge.py tests/test_model_ops_cheap_first_release_decision.py tests/test_model_ops_readiness.py tests/test_release_readiness.py tests/test_continuous_update_ledger.py tests/test_frontend_ui_regression_gate.py -q && cd ../frontend && npm run typecheck && npm run ui:regression",
                manual_note="This is metadata-only legal benchmark default-promotion bridge evidence joining legal fixture benchmark gate, default-promotion packet, regression budget, evidence handoff, and Gemini official lifecycle drift gate before any cheap-first legal default review; it does not call NewAPI, Gemini, OpenAI, Google, gateways, app AI endpoints, public datasets, or the network, write configuration, shift traffic, claim maintainer approval, claim public benchmark scores, or return raw legal text, fixture snippets, prompts, model outputs, gateway payloads, emails, identifiers, credentials, or secrets.",
            ),
            ReleaseCheck(
                ...
                validation_command="python -m pytest tests/test_modelops_legal_benchmark_default_promotion_checklist.py tests/test_modelops_legal_benchmark_default_promotion_bridge.py tests/test_model_ops_readiness.py tests/test_release_readiness.py tests/test_continuous_update_ledger.py tests/test_frontend_ui_regression_gate.py -q && cd ../frontend && npm run typecheck && npm run ui:regression",
                manual_note="This is metadata-only legal benchmark default-promotion checklist evidence joining the default-promotion bridge, cheap-first release decision, and default-change queue for maintainer signoff review; it does not call NewAPI, Gemini, OpenAI, Google, gateways, app AI endpoints, public datasets, or the network, write configuration, write env files, record approvals, shift traffic, claim maintainer approval, claim public benchmark scores, or return raw legal text, fixture snippets, prompts, model outputs, gateway payloads, emails, identifiers, credentials, or secrets.",
            ),
            ReleaseCheck(
                ...
                validation_command="python -m pytest tests/test_modelops_legal_benchmark_default_promotion_signoff_packet.py tests/test_modelops_legal_benchmark_default_promotion_checklist.py tests/test_model_ops_readiness.py tests/test_release_readiness.py tests/test_continuous_update_ledger.py tests/test_frontend_ui_regression_gate.py -q && cd ../frontend && npm run typecheck && npm run ui:regression",
                manual_note="This is metadata-only legal benchmark default-promotion signoff packet evidence packaging checklist rows into external maintainer signoff requirements; it does not call NewAPI, Gemini, OpenAI, Google, gateways, app AI endpoints, public datasets, or the network, write configuration, write env files, collect approver identity, write approval or signoff records, shift traffic, claim maintainer approval, claim public benchmark scores, or return raw legal text, fixture snippets, prompts, model outputs, gateway payloads, emails, identifiers, credentials, or secrets.",
            ),
            ReleaseCheck(
                ...
                validation_command="python -m pytest tests/test_modelops_legal_benchmark_default_promotion_execution_handoff.py tests/test_modelops_legal_benchmark_default_promotion_signoff_packet.py tests/test_model_ops_readiness.py tests/test_release_readiness.py tests/test_continuous_update_ledger.py tests/test_frontend_ui_regression_gate.py -q && cd ../frontend && npm run typecheck && npm run ui:regression",
                manual_note="This is metadata-only legal benchmark default-promotion execution handoff and rollback gate evidence packaging externally signed rows into execution prerequisites, rollback checks, config diff review, and post-change observation requirements; it does not call NewAPI, Gemini, OpenAI, Google, gateways, app AI endpoints, public datasets, or the network, write configuration, write env files, collect approver identity, write approval or signoff records, execute rollback, shift traffic, claim maintainer approval, claim public benchmark scores, or return raw legal text, fixture snippets, prompts, model outputs, gateway payloads, emails, identifiers, credentials, or secrets.",
            ),
            ReleaseCheck(
                ...
                validation_command="python -m pytest tests/test_modelops_legal_benchmark_default_promotion_observation_gate.py tests/test_modelops_legal_benchmark_default_promotion_execution_handoff.py tests/test_model_ops_readiness.py tests/test_release_readiness.py tests/test_continuous_update_ledger.py tests/test_frontend_ui_regression_gate.py -q && cd ../frontend && npm run typecheck && npm run ui:regression",
                manual_note="This is metadata-only legal benchmark default-promotion post-execution observation gate evidence packaging externally supplied route telemetry, legal benchmark smoke, rollback-window, and incident-status metadata after execution handoff; it does not call NewAPI, Gemini, OpenAI, Google, gateways, app AI endpoints, public datasets, or the network, write configuration, write env files, collect approver identity, write approval or signoff records, execute rollback, shift traffic, claim post-change production quality, claim public benchmark scores, or return raw legal text, fixture snippets, prompts, model outputs, gateway payloads, emails, identifiers, credentials, or secrets.",
            ),
            ReleaseCheck(
                ...
                validation_command="python -m pytest tests/test_model_ops_cheap_first_release_decision.py tests/test_model_ops_readiness.py tests/test_model_catalog_source_audit.py tests/test_model_route_quality_budget.py tests/test_model_ops_cheap_first_escalation_budget.py tests/test_model_failure_upgrade_budget.py tests/test_modelops_legal_fixture_cheap_first_benchmark_gate.py tests/test_modelops_legal_fixture_default_promotion_packet.py tests/test_modelops_legal_fixture_cheap_first_regression_budget.py tests/test_modelops_legal_benchmark_default_promotion_bridge.py tests/test_model_ops_legal_benchmark_risk_bridge.py tests/test_model_default_candidate_selector.py -q",
                manual_note="This is a metadata-only cheap-first release decision packet that consumes legal fixture benchmark gate, legal fixture default-promotion packet, legal fixture regression budget, legal benchmark default-promotion bridge, and legal benchmark route-risk bridge evidence; it does not call NewAPI, Gemini, OpenAI, Google, any gateway, download public benchmark data, write configuration, shift traffic, return raw legal text, or claim public benchmark scores.",
            ),
            ReleaseCheck(
            ...
            ReleaseCheck(
                ...
                validation_command="python -m pytest tests/test_model_ops_request_execution_preflight.py tests/test_model_runtime_router.py tests/test_model_request_cost_bounds.py tests/test_model_gateway_request_compatibility_gate.py tests/test_model_ops_readiness.py tests/test_frontend_ui_regression_gate.py -q && cd ../frontend && npm run typecheck && npm run ui:regression",
                manual_note="This is metadata-only per-request execution preflight release evidence for sanitized NewAPI/Gemini request metadata. It joins runtime model resolution, cheap-first fallback ordering, estimated input/output token costs, task cost bounds, max_tokens policy, local downgrade visibility, and ModelOps UI review before live calls; it does not call NewAPI, Gemini, OpenAI, Google, gateways, app AI endpoints, models, or the network, does not write configuration, change defaults, or shift traffic, and does not include headers, request bodies, messages, prompts, raw legal text, raw payloads, model outputs, gateway responses, emails, user identifiers, or credentials.",
            ),
            ReleaseCheck(
                ...
                validation_command="python -m pytest tests/test_model_ops_request_execution_observation_gate.py tests/test_model_ops_request_execution_preflight.py tests/test_model_ops_readiness.py tests/test_frontend_ui_regression_gate.py -q && cd ../frontend && npm run typecheck && npm run ui:regression",
                manual_note="This is optional metadata-only post-run request observation evidence for sanitized NewAPI/Gemini execution metadata. It compares observations with request execution preflight rows, cheap-first model alignment, fallback use, observed token/cost/latency metadata, coarse error categories, and local downgrade follow-through without executing requests; it does not call NewAPI, Gemini, OpenAI, Google, gateways, app AI endpoints, models, or the network, does not write configuration, change defaults, or shift traffic, and does not include headers, request bodies, messages, prompts, raw legal text, raw payloads, gateway responses, model outputs, emails, user identifiers, or credentials.",
            ),
            ReleaseCheck(
            ...
            ReleaseCheck(
                ...
                title="Route telemetry result archive and cost ledger coverage",
                category="model_ops",
                required=True,
                owner="engineering",
                evidence_paths=(
                ...
                validation_command="python -m pytest tests/test_route_telemetry_result_archive.py tests/test_release_readiness.py tests/test_continuous_update_ledger.py tests/test_frontend_ui_regression_gate.py -q && cd ../frontend && npm run typecheck && npm run ui:regression",
                manual_note="This exposes metadata-only route telemetry result archive and cost ledger evidence through maintenance, AIHub, ModelOps readiness, and the /model-ops UI; it does not call NewAPI, Gemini, OpenAI, Google, gateways, app AI endpoints, models, or the network, does not write configuration, change default routes, or shift traffic, does not claim production health or public benchmark scores, and excludes raw events, prompts, legal text, request bodies, response bodies, headers, gateway responses, model outputs, credentials, emails, and user identifiers.",
            ),
            ReleaseCheck(
            ...
            ReleaseCheck(
                ...
                validation_command="python -m pytest tests/test_model_ops_gentxt_task_guard.py tests/test_model_task_inference.py tests/test_aihub_runtime_routing.py tests/test_model_ops_readiness.py tests/test_frontend_ui_regression_gate.py -q",
                manual_note="This is metadata-only gentxt routing guard evidence for rejecting media and speech routing aliases on the text endpoint while keeping explicit media defaults scoped to media endpoints; it does not call NewAPI, Gemini, OpenAI, Google, gateways, app AI endpoints, models, or the network, does not write configuration or shift traffic, and does not include request bodies, response bodies, headers, prompts, raw payloads, legal text, model outputs, gateway responses, credentials, emails, or user identifiers.",
            ),
            ReleaseCheck(
            ...
            ReleaseCheck(
                ...
                validation_command="python -m pytest tests/test_legal_fixture_regression.py tests/test_legal_fixture_run_report.py tests/test_continuous_update_ledger.py -q",
                manual_note="This compares baseline and current cheap-first legal fixture runs with metadata-only score, status, escalation, and cost deltas. A failed comparison blocks release candidates, but not-run optional evidence does not block ordinary releases. It does not call NewAPI, Gemini, OpenAI, Google, gateways, models, public datasets, or the network, and it does not return raw model outputs, gateway responses, prompts, legal text, client documents, emails, headers, payload bodies, credentials, or secrets.",
            ),
            ReleaseCheck(
            ...
            ReleaseCheck(
                ...
                validation_command=(
                    "python -m pytest tests/test_legal_document_benchmark_route_plan_execution_result_archive.py "
                    "tests/test_legal_document_benchmark_route_plan_execution_readiness.py "
                    "tests/test_legal_document_benchmark_route_plan.py tests/test_release_readiness.py "
                    "tests/test_continuous_update_ledger.py tests/test_maintenance_evidence.py "
                    "tests/test_frontend_ui_regression_gate.py -q && cd ../frontend && npm run typecheck && "
                    "npm run ui:regression"
                ),
                manual_note=(
                    "This builds a metadata-only legal-document benchmark route-plan execution result archive "
                    "for externally supplied manual observation metadata after execution readiness. It checks "
                    "case_id, phase, observed model, status, token, cost, latency, fallback, and coarse error "
                    "metadata against the cheap-first route plan with fixture_limit=3 and max_parallel_model_requests=1, "
                    "but does not call NewAPI, Gemini, gateways, app AI endpoints, models, public datasets, or the "
                    "network; execute benchmark runs; write archive files; record maintainer approval; change "
                    "defaults; shift traffic; write configuration; or return public benchmark text, raw fixture "
                    "snippets, prompts, request bodies, response bodies, headers, raw legal text, generated "
                    "document text, model outputs, gateway responses, credentials, emails, or client identifiers."
                ),
            ),
            ReleaseCheck(
                ...
                validation_command=(
                    "python -m pytest tests/test_legal_document_benchmark_route_plan_execution_result_handoff.py "
                    "tests/test_legal_document_benchmark_route_plan_execution_result_archive.py "
                    "tests/test_legal_document_benchmark_route_plan_execution_readiness.py "
                    "tests/test_release_readiness.py tests/test_continuous_update_ledger.py "
                ...
                manual_note=(
                    "This builds a metadata-only legal-document benchmark route-plan execution result handoff "
                    "that decides whether sanitized manual observation archives can be attached as release "
                    "evidence after execution readiness. It requires a ready readiness packet, a ready result "
                    "archive, cheap-first model alignment, fixture_limit=3, and max_parallel_model_requests=1, "
                    "but does not call NewAPI, Gemini, gateways, app AI endpoints, models, public datasets, or "
                    "the network; execute benchmark runs; write release records or archive files; record "
                    "maintainer approval; claim public benchmark scores; change defaults; shift traffic; write "
                    "configuration; or return public benchmark text, raw fixture snippets, prompts, request "
                    "bodies, response bodies, headers, raw legal text, generated document text, model outputs, "
                    "gateway responses, credentials, emails, or client identifiers."
                ),
            ),
            ReleaseCheck(
                ...
                validation_command=(
                    "python -m pytest tests/test_legal_document_benchmark_route_plan_execution_review_packet.py "
                    "tests/test_legal_document_benchmark_route_plan_execution_result_handoff.py "
                    "tests/test_legal_document_benchmark_route_plan_execution_result_archive.py "
                    "tests/test_legal_document_benchmark_route_plan_execution_readiness.py "
                    "tests/test_release_readiness.py tests/test_continuous_update_ledger.py "
                    "tests/test_maintenance_evidence.py tests/test_frontend_ui_regression_gate.py -q && "
                ...
                manual_note=(
                    "This builds a metadata-only legal-document benchmark route-plan execution review packet "
                    "that summarizes readiness, sanitized execution-result archive, and release-evidence handoff "
                    "for reviewer attach/review/hold decisions. It requires ready readiness, ready archive, "
                    "ready handoff, cheap-first model alignment, fixture_limit=3, and "
                    "max_parallel_model_requests=1 before a packet can be attached as release evidence, but "
                    "does not call NewAPI, Gemini, gateways, app AI endpoints, models, public datasets, or the "
                    "network; execute benchmark runs; write release records or archive files; record maintainer "
                    "approval; claim public benchmark scores; change defaults; shift traffic; write configuration; "
                    "or return public benchmark text, raw fixture snippets, prompts, request bodies, response "
                    "bodies, headers, raw legal text, generated document text, model outputs, gateway responses, "
                    "credentials, emails, or client identifiers."
                ),
            ),
            ReleaseCheck(
                ...
                validation_command=(
                    "python -m pytest tests/test_legal_document_benchmark_route_plan_execution_claim_gate.py "
                    "tests/test_legal_document_benchmark_route_plan_execution_review_packet.py "
                    "tests/test_release_readiness.py tests/test_continuous_update_ledger.py "
                    "tests/test_maintenance_evidence.py tests/test_frontend_ui_regression_gate.py -q && "
                    "cd ../frontend && npm run typecheck && npm run ui:regression"
                ),
                manual_note=(
                    "This builds a metadata-only legal-document benchmark route-plan execution claim gate "
                    "that hashes proposed release/support claims and allows only ready review-packet-backed "
                    "metadata-only wording. It blocks public benchmark scores, live NewAPI/Gemini execution "
                    "claims, maintainer or release approval claims, default changes, traffic shifts, production "
                    "quality claims, and sensitive material, without echoing raw claim text; calling NewAPI, "
                    "Gemini, gateways, app AI endpoints, models, public datasets, or the network; executing "
                    "benchmark runs; writing release records, archive files, configuration, defaults, or "
                    "traffic shifts; recording approval; or returning public benchmark text, raw legal text, "
                    "prompts, request bodies, response bodies, headers, model outputs, gateway responses, "
                    "credentials, emails, or client identifiers."
                ),
            ),
            ReleaseCheck(
            ...
            ReleaseCheck(
                ...
                evidence_paths=(
                    ...
                    "app/backend/services/legal_document_benchmark_suite.py",
                    "app/backend/services/legal_document_fact_consistency_benchmark.py",
                    "app/backend/services/final_document_delivery_release_gate.py",
                    "app/backend/routers/maintenance.py",
                    "app/frontend/src/lib/maintenanceApi.ts",
                    "app/frontend/src/pages/MaintenanceEvidencePage.tsx",
                    "app/frontend/scripts/ui-regression.mjs",
                ...
                validation_command="python -m pytest tests/test_small_legal_document_benchmark_runbook_evidence.py tests/test_legal_document_benchmark_suite.py tests/test_legal_document_fact_consistency_benchmark.py tests/test_final_document_delivery_release_gate.py tests/test_release_readiness.py tests/test_continuous_update_ledger.py tests/test_maintenance_evidence.py tests/test_frontend_ui_regression_gate.py -q && cd ../frontend && npm run typecheck && npm run ui:regression",
                manual_note="This is metadata-only small legal-document benchmark runbook evidence for structure/citation checks, fact consistency checks, and final delivery gates before client delivery. It does not call NewAPI, Gemini, OpenAI, Google, gateways, app AI endpoints, models, public datasets, or the network; download public benchmark samples; write configuration; shift traffic; claim public benchmark scores, production legal quality, client-document coverage, generated final files, or sent delivery; or return raw legal text, document snippets, fixture snippets, generated text, prompts, payload bodies, model outputs, gateway responses, credentials, emails, identifiers, or client material.",
            ),
            ReleaseCheck(
            ...
            ReleaseCheck(
                ...
                title="Legal public fixture priority queue",
                category="quality",
                required=False,
                owner="quality",
                evidence_paths=(
                ...
                validation_command="python -m pytest tests/test_legal_public_fixture_priority_queue.py tests/test_legal_public_benchmark_sampler.py tests/test_legal_benchmark_fixture_crosswalk.py tests/test_user_need_benchmark_coverage.py tests/test_frontend_ui_regression_gate.py -q && cd ../frontend && npm run typecheck && npm run ui:regression",
                manual_note="This is metadata-only public benchmark to synthetic fixture priority evidence; it maps LawBench, LexEval, LegalBench, LegalBench-RAG, CUAD, LexGLUE, CaseGen, and corpus-scale references into local synthetic fixture work without downloading datasets, importing public benchmark text, claiming public benchmark scores, calling NewAPI, Gemini, OpenAI, Google, gateways, models, or the network, writing configuration, returning raw legal text, fixture snippets, small-corpus excerpts, prompts, model outputs, payloads, or credentials.",
            ),
            ReleaseCheck(
                id="user-need-legal-document-benchmark-evidence",
                title="User need legal-document benchmark evidence",
                category="quality",
                required=False,
                owner="quality",
                evidence_paths=(
                ...
                validation_command="python -m pytest tests/test_user_need_legal_document_benchmark_evidence.py tests/test_user_need_benchmark_coverage.py tests/test_legal_document_benchmark_suite.py tests/test_legal_document_benchmark_coverage.py tests/test_legal_document_benchmark_fixtures.py tests/test_legal_document_fact_consistency_benchmark.py tests/test_modelops_legal_fixture_cheap_first_benchmark_gate.py tests/test_frontend_ui_regression_gate.py -q && cd ../frontend && npm run typecheck && npm run ui:regression",
                manual_note="This is metadata-only user-need to legal-document benchmark evidence; it joins local synthetic document benchmark cases, fact consistency checks, local rule baseline, and cheap-first gate status without downloading public datasets, importing public benchmark text, claiming public benchmark scores or production legal quality, calling NewAPI, Gemini, OpenAI, Google, gateways, models, or the network, changing default models, returning raw legal text, fixture snippets, document snippets, prompts, model outputs, payload bodies, or credentials.",
            ),
            ReleaseCheck(
                ...
                validation_command="python -m pytest tests/test_feedback_user_need_legal_document_benchmark_backlog.py tests/test_feedback_issue_cluster.py tests/test_feedback_roadmap_alignment.py tests/test_user_need_legal_document_benchmark_evidence.py tests/test_user_need_implementation_priority_queue.py tests/test_frontend_ui_regression_gate.py -q && cd ../frontend && npm run typecheck && npm run ui:regression",
                manual_note="This is metadata-only feedback to user-need legal-document benchmark backlog evidence; it clusters privacy-safe feedback metadata, maps clusters to user needs, and joins legal-document benchmark evidence without storing or returning raw feedback, customer notes, PII, uploaded document text, fixture snippets, public benchmark text, prompts, payload bodies, model outputs, gateway responses, credentials, claiming feedback resolution, public benchmark scores, production quality, or client-document coverage.",
            ),
            ReleaseCheck(
                ...
                validation_command="python -m pytest tests/test_feedback_user_need_legal_document_benchmark_release_packet.py tests/test_feedback_user_need_legal_document_benchmark_backlog.py tests/test_feedback_lifecycle_policy.py tests/test_user_need_implementation_priority_queue.py tests/test_frontend_ui_regression_gate.py -q && cd ../frontend && npm run typecheck && npm run ui:regression",
                manual_note="This is metadata-only feedback user-need legal-document benchmark release packet evidence; it joins privacy-safe feedback benchmark backlog rows to feedback lifecycle checks, implementation queue status, legal-document benchmark evidence, release observations, and customer-resolution gates without returning raw feedback, customer notes, public resolution text, PII, uploaded document text, fixture snippets, public benchmark text, prompts, payload bodies, gateway responses, model outputs, credentials, claiming feedback resolution, customer notification, production legal quality, public benchmark scores, or client-document coverage.",
            ),
            ReleaseCheck(
            ...
            ReleaseCheck(
                ...
                validation_command="python -m pytest tests/test_user_need_gemini_route_coverage.py tests/test_release_readiness.py tests/test_continuous_update_ledger.py tests/test_maintenance_evidence.py tests/test_frontend_ui_regression_gate.py -q && cd ../frontend && npm run typecheck && npm run ui:regression",
                manual_note="This is metadata-only user-need Gemini route coverage evidence that joins user-need benchmark coverage, cheap-first calibration tasks, and Gemini cheap-first route preflight evidence; it is visible through /api/v1/maintenance/user-needs/gemini-route-coverage, /api/v1/aihub/models/user-need-gemini-route-coverage, /maintenance, and /model-ops, and shows Flash-Lite protected needs, review-required premium/benchmark/license gaps, and unmapped route blockers without downloading public datasets, importing public benchmark samples, calling NewAPI, Gemini, OpenAI, Google, gateways, app AI endpoints, or the network, writing configuration or default routes, shifting traffic, or including raw legal text, prompts, route payloads, request bodies, response bodies, headers, model outputs, gateway responses, credentials, emails, or user identifiers.",
            ),
            ReleaseCheck(
            ...
            ReleaseCheck(
                ...
                manual_note="This is metadata-only AIHub endpoint route coverage gate evidence for endpoint inventory including embeddings, runtime-router coverage, budget-decision coverage, route telemetry coverage, response route payload coverage, streaming SSE metadata coverage, task inference response coverage, embedding/media usage-unit coverage, and media/speech/embedding catalog review gaps; it does not call NewAPI, Gemini, OpenAI, Google, gateways, app AI endpoints, models, or the network, does not write configuration, shift traffic, or write embedding indexes, and does not include request bodies, response bodies, headers, prompts, raw payloads, legal text, embedding vectors, model outputs, gateway responses, credentials, emails, or user identifiers.",
            ),
            ReleaseCheck(
                ...
                validation_command="python -m pytest tests/test_model_ops_aihub_media_speech_default_catalog_gate.py tests/test_model_ops_aihub_endpoint_route_coverage_gate.py tests/test_model_ops_readiness.py tests/test_release_readiness.py tests/test_continuous_update_ledger.py tests/test_frontend_ui_regression_gate.py -q && cd ../frontend && npm run typecheck && npm run ui:regression",
                manual_note="This is metadata-only AIHub media/speech default catalog gate evidence for image, video, audio, transcription, future Live audio, and embedding default-review coverage at /api/v1/aihub/models/aihub-media-speech-default-catalog-gate; it links endpoint route coverage, local catalog status, budget modes, official Gemini/Veo/TTS source anchors, default release actions, review items, and privacy/claim boundaries while keeping non-catalog and future-route defaults explicit-review only. It does not call NewAPI, Gemini, OpenAI, Google, gateways, app AI endpoints, models, or the network, does not write configuration, change defaults, or shift traffic, and does not include request bodies, response bodies, headers, prompts, raw payloads, audio, transcripts, legal text, model outputs, gateway responses, credentials, emails, or user identifiers.",
            ),
            ReleaseCheck(
                ...
                validation_command="python -m pytest tests/test_model_ops_aihub_media_runtime_compatibility_gate.py tests/test_model_ops_aihub_media_speech_default_catalog_gate.py tests/test_model_ops_readiness.py tests/test_release_readiness.py tests/test_continuous_update_ledger.py tests/test_frontend_ui_regression_gate.py -q && cd ../frontend && npm run typecheck && npm run ui:regression",
                manual_note="This is metadata-only AIHub media runtime compatibility evidence at /api/v1/aihub/models/aihub-media-runtime-compatibility-gate for genvideo, genaudio, transcribe, and future Live audio route shapes. It separates current OpenAI-compatible client.videos.create/retrieve, client.audio.speech.create, and client.audio.transcriptions.create code paths from native Gemini/Veo/TTS/Live runtime requirements, keeping Veo, Gemini TTS, Gemini audio-understanding, and Live audio promotion review-only until gateway shape, native adapter, polling, session, and output-extraction evidence is attached. It does not call NewAPI, Gemini, OpenAI, Google, gateways, app AI endpoints, models, or the network, does not write configuration, change defaults, or shift traffic, and does not include request bodies, response bodies, headers, prompts, raw payloads, audio, transcripts, legal text, model outputs, gateway responses, credentials, emails, or user identifiers.",
            ),
            ReleaseCheck(
                ...
                validation_command="python -m pytest tests/test_model_ops_gemini_embedding_cheap_first_preflight.py tests/test_model_catalog.py tests/test_model_budget.py tests/test_model_configuration_audit.py tests/test_model_ops_readiness.py tests/test_release_readiness.py tests/test_continuous_update_ledger.py tests/test_frontend_ui_regression_gate.py -q && cd ../frontend && npm run typecheck && npm run ui:regression",
                manual_note="This is metadata-only Gemini embedding cheap-first preflight evidence at /api/v1/aihub/models/gemini-embedding-cheap-first-preflight for APP_AI_EMBEDDING_MODEL=gemini-embedding-001 as the default text embedding route, auto-embedding alias coverage, local catalog pricing, cheap-first budget policy, and review routing for multimodal gemini-embedding-2. Multimodal gemini-embedding-2 remains review-required before any image, audio, video, PDF, or source-index use. It does not call NewAPI, Gemini, OpenAI, Google, gateways, app AI endpoints, models, or the network, does not write configuration, change defaults, write indexes, or shift traffic, and does not include source text, raw legal text, source chunks, embedding vectors, request bodies, response bodies, headers, prompts, raw payloads, model outputs, gateway responses, credentials, emails, or user identifiers.",
            ),
            ReleaseCheck(
                ...
                validation_command="python -m pytest tests/test_modelops_legal_micro_benchmark_preflight.py tests/test_legal_fixture_local_run_package.py tests/test_modelops_legal_fixture_cheap_first_benchmark_gate.py tests/test_model_ops_readiness.py -q",
                manual_note="This is metadata-only low-resource legal benchmark preflight evidence for cheap-first Gemini fixture selection, document case ids, fact-consistency case ids, serial run order, cost estimates, and follow-up gate bindings; it does not call NewAPI, Gemini, OpenAI, Google, gateways, app AI endpoints, or the network, does not write configuration or shift traffic, and does not include request bodies, messages, prompts, fixture excerpts, legal text, generated document text, model outputs, gateway responses, credentials, or emails.",
            ),
            ReleaseCheck(
                ...
                manual_note="This is metadata-only small legal-document cheap-first Gemini benchmark/risk gate evidence exposed in the AIHub ModelOps payload and ModelOps UI using redacted fixture ids, document case ids, fact-consistency case ids, local rule baseline case ids and match counts, linked cheap-first calibration task ids, calibration decisions, release gates, expected issue counts, amount/date/fact consistency counts, cost metadata, document benchmark pass/fail counts, coverage-gap counts, and escalation metadata only; it does not call NewAPI, Gemini, OpenAI, Google, gateways, or the network, and it does not include real legal text, fixture snippets, local rule predictions, extracted field values, prompts, generated document text, calibration payloads, model outputs, credentials, or emails.",
            ),
            ReleaseCheck(
                ...
                manual_note="This is metadata-only maintainer review packet evidence exposed in the AIHub ModelOps payload and ModelOps UI for cheap-first legal fixture default promotion using fixture ids, document case ids, fact-consistency case ids, local rule baseline status and match counts, linked cheap-first calibration task ids, calibration decisions, release gates, gate status, document benchmark pass/fail counts, amount/date/fact consistency counts, coverage-gap counts, cost-tier metadata, reason codes, and required signoff roles only; it never writes configuration, never calls NewAPI, Gemini, OpenAI, Google, gateways, or the network, never shifts traffic, and does not include real legal text, fixture snippets, local rule predictions, extracted field values, prompts, generated document text, calibration payloads, model outputs, credentials, or emails.",
            ),
            ReleaseCheck(
                ...
                validation_command="python -m pytest tests/test_modelops_legal_fixture_cheap_first_regression_budget.py tests/test_legal_fixture_regression.py tests/test_model_ops_cheap_first_release_decision.py tests/test_model_ops_readiness.py tests/test_release_readiness.py tests/test_continuous_update_ledger.py tests/test_maintenance_evidence.py -q && cd ../frontend && npm run typecheck && npm run ui:regression",
                manual_note="This is metadata-only low-resource regression budget evidence for cheap-first legal fixture default reviews. It joins fixture regression comparison, cheap-first benchmark gate, default-promotion packet, and small-document runbook status using fixture ids, source statuses, counts, cost deltas, reason codes, max_parallel_requests=1, and review actions only; it does not call NewAPI, Gemini, OpenAI, Google, gateways, public datasets, or the network, does not write configuration or shift traffic, and does not include raw legal text, fixture snippets, prompts, generated document text, model outputs, gateway payloads, credentials, emails, or approvals.",
            ),
            ReleaseCheck(
                ...
                validation_command="python -m pytest tests/test_modelops_legal_fixture_evidence_handoff.py tests/test_legal_fixture_local_run_review.py tests/test_modelops_legal_fixture_cheap_first_benchmark_gate.py tests/test_continuous_session_run_monitor.py tests/test_model_ops_readiness.py -q",
                manual_note="This is metadata-only archive-safe legal fixture evidence handoff for local-run-review, cheap-first benchmark gate, default-promotion packet, and continuous-session-run-monitor summaries. It returns source statuses, fixture counts, readiness/review/blocker counts, endpoint links, and privacy/claim boundaries only; it does not return run_report_payload, observations, output_text, raw gateway responses, prompts, messages, headers, credentials, raw legal text, model outputs, NewAPI/Gemini/OpenAI/Google/gateway/network results, configuration writes, default changes, traffic shifts, 24-hour completion claims, 100-update completion claims, GitHub push claims, or default-change claims.",
            ),
            ReleaseCheck(
            ...
            ReleaseCheck(
                ...
                validation_command="python -m pytest tests/test_model_ops_observed_gemini_coverage_gap_queue.py tests/test_model_ops_observed_gemini_model_intake_queue.py tests/test_gemini_model_variant_matrix.py tests/test_release_readiness.py tests/test_continuous_update_ledger.py tests/test_maintenance_evidence.py -q",
                manual_note="This is metadata-only ModelOps observed Gemini coverage gap evidence for checking Gemini family coverage, high-frequency cheap-first task coverage, unknown/unpriced/preview/media risk, and default-promotion review actions from sanitized observed model ids; it does not call NewAPI, Gemini, OpenAI, Google, gateways, or the network, does not write real environment values, and does not include raw prompts, payloads, model outputs, or credentials.",
            ),
            ReleaseCheck(
                ...
                validation_command="python -m pytest tests/test_model_ops_observed_gemini_premium_exception_review.py tests/test_model_ops_observed_gemini_coverage_gap_queue.py tests/test_model_ops_observed_gemini_model_intake_queue.py tests/test_release_readiness.py tests/test_continuous_update_ledger.py tests/test_maintenance_evidence.py -q",
                manual_note="This is metadata-only ModelOps observed Gemini premium exception review evidence for Pro or premium Gemini variants; it supports explicit premium routes only after maintainer review, blocks high-frequency defaults and automatic configuration changes, and does not call NewAPI, Gemini, OpenAI, Google, gateways, or the network, write real environment values, shift traffic, or include raw prompts, payloads, model outputs, legal text, emails, or credentials.",
            ),
            ReleaseCheck(
            ...
            ReleaseCheck(
                ...
                validation_command="python -m pytest tests/test_legal_rag_index_coverage_gate.py tests/test_legal_rag_index_binding.py tests/test_release_readiness.py tests/test_continuous_update_ledger.py tests/test_maintenance_evidence.py tests/test_frontend_ui_regression_gate.py -q && cd ../frontend && npm run typecheck && npm run ui:regression",
                manual_note="This is metadata-only Legal RAG index coverage gate evidence for index binding plan rows, filter validation, source coverage, retrieval locator coverage, jurisdiction/freshness, cheap-first review actions, typed maintenance API helpers, and maintenance UI review; it does not call NewAPI, Gemini, models, gateways, or the network, download datasets, store or return source ids, raw query, raw retrieved context, raw legal text, prompts, model outputs, gateway payloads, or credentials.",
            ),
            ReleaseCheck(
                ...
                validation_command="python -m pytest tests/test_legal_rag_embedding_readiness_gate.py tests/test_model_ops_gemini_embedding_cheap_first_preflight.py tests/test_legal_rag_index_coverage_gate.py tests/test_legal_rag_retrieval_diagnostics_gate.py tests/test_release_readiness.py tests/test_continuous_update_ledger.py tests/test_maintenance_evidence.py tests/test_frontend_ui_regression_gate.py -q && cd ../frontend && npm run typecheck && npm run ui:regression",
                manual_note="This is metadata-only Legal RAG embedding readiness gate evidence for Gemini embedding cheap-first defaults, text-only index preflight rows, multimodal embedding review boundaries, index coverage blockers, retrieval diagnostics linkage, typed maintenance API helpers, and maintenance UI review; it does not call NewAPI, Gemini, models, gateways, or the network, write indexes, download datasets, store or return source ids, raw query, raw retrieved context, raw legal text, embedding vectors, prompts, model outputs, gateway payloads, or credentials.",
            ),
            ReleaseCheck(
                ...
                validation_command="python -m pytest tests/test_legal_rag_embedding_chunk_policy_gate.py tests/test_legal_rag_embedding_readiness_gate.py tests/test_legal_source_durable_index_plan.py tests/test_release_readiness.py tests/test_continuous_update_ledger.py tests/test_maintenance_evidence.py tests/test_frontend_ui_regression_gate.py -q && cd ../frontend && npm run typecheck && npm run ui:regression",
                manual_note="This is metadata-only Legal RAG embedding chunk policy gate evidence for token-estimate chunk planning, source-type split strategies, citation-anchor checks, retrieval-locator blockers, freshness review boundaries, cheap Gemini embedding defaults, typed maintenance API helpers, and maintenance UI review; it does not call NewAPI, Gemini, models, gateways, or the network, create embeddings, write indexes, download datasets, store or return source ids, raw query, raw retrieved context, raw legal text, source chunks, embedding vectors, prompts, model outputs, gateway payloads, or credentials.",
            ),
            ReleaseCheck(
                ...
                validation_command="python -m pytest tests/test_legal_rag_embedding_index_dry_run_gate.py tests/test_legal_rag_embedding_chunk_policy_gate.py tests/test_legal_source_durable_index_plan.py tests/test_release_readiness.py tests/test_continuous_update_ledger.py tests/test_maintenance_evidence.py tests/test_frontend_ui_regression_gate.py -q && cd ../frontend && npm run typecheck && npm run ui:regression",
                manual_note="This is metadata-only Legal RAG embedding index dry-run evidence for reviewing planned vector slots, chunk-policy readiness, durable index persistence fields, and repository validation before any embedding index write; it does not call NewAPI, Gemini, models, gateways, or the network, create embeddings, write indexes or databases, download datasets, store or return source ids, raw query, raw retrieved context, raw legal text, source chunks, embedding vectors, prompts, model outputs, gateway payloads, or credentials.",
            ),
            ReleaseCheck(
                ...
                validation_command="python -m pytest tests/test_legal_rag_embedding_batch_budget_gate.py tests/test_legal_rag_embedding_index_dry_run_gate.py tests/test_model_ops_gemini_embedding_cheap_first_preflight.py tests/test_release_readiness.py tests/test_continuous_update_ledger.py tests/test_maintenance_evidence.py tests/test_frontend_ui_regression_gate.py -q && cd ../frontend && npm run typecheck && npm run ui:regression",
                manual_note="This is metadata-only Legal RAG embedding batch budget evidence for reviewing cheap Gemini embedding batch splits, low-resource local queue limits, planned chunk counts, estimated token totals, and local catalog batch-cost estimates before any embedding run; it does not call NewAPI, Gemini, models, gateways, or the network, create embeddings, write indexes or databases, download datasets, store or return source ids, raw query, raw retrieved context, raw legal text, source chunks, embedding vectors, prompts, model outputs, gateway payloads, or credentials.",
            ),
            ReleaseCheck(
            ...
            ReleaseCheck(
                ...
                validation_command="python -m pytest tests/test_legal_rag_embedding_batch_preview.py tests/test_legal_rag_router.py tests/test_aihub_runtime_routing.py tests/test_release_readiness.py tests/test_continuous_update_ledger.py tests/test_maintenance_evidence.py -q",
                manual_note="This is executable Legal RAG embedding batch preview evidence for maintainer-only laptop-safe smoke checks through the configured AIHub embedding runtime and cheap-first Gemini routing; it can call the configured gateway/model when invoked, but it never writes indexes or databases and must not return source text, source ids, embedding vectors, prompts, gateway payloads, model outputs, credentials, or legal advice claims.",
            ),
            ReleaseCheck(
                ...
                validation_command="python -m pytest tests/test_legal_rag_embedding_batch_approval_packet.py tests/test_legal_rag_embedding_batch_budget_gate.py tests/test_release_readiness.py tests/test_continuous_update_ledger.py tests/test_maintenance_evidence.py tests/test_frontend_ui_regression_gate.py -q && cd ../frontend && npm run typecheck && npm run ui:regression",
                manual_note="This is metadata-only Legal RAG embedding batch approval packet evidence for reviewing serial low-resource run order, max_parallel_embedding_requests=1, required maintainer/RAG-index signoff roles, pre-approval checks, and advance/hold/block actions before any cheap Gemini embedding run; it does not claim approval, collect approver identity, write approval records, call NewAPI, Gemini, models, gateways, or the network, create embeddings, write indexes or databases, download datasets, store or return source ids, raw query, raw retrieved context, raw legal text, source chunks, embedding vectors, prompts, model outputs, gateway payloads, or credentials.",
            ),
            ReleaseCheck(
                ...
                validation_command="python -m pytest tests/test_legal_rag_embedding_batch_observation_gate.py tests/test_legal_rag_embedding_batch_approval_packet.py tests/test_release_readiness.py tests/test_continuous_update_ledger.py tests/test_maintenance_evidence.py tests/test_frontend_ui_regression_gate.py -q && cd ../frontend && npm run typecheck && npm run ui:regression",
                manual_note="This is metadata-only Legal RAG embedding batch observation gate evidence for reviewing sanitized aggregate observations after an external cheap Gemini embedding batch run, observed batch/chunk/vector-slot/token counts, cost deltas, max_parallel_embedding_requests=1, and allow/hold/block index-review actions; it does not claim maintainer approval, execute embeddings, call NewAPI, Gemini, models, gateways, or the network, write indexes or databases, download datasets, collect approver identity, store or return source ids, approval item ids, raw query, raw retrieved context, raw legal text, source chunks, embedding vectors, prompts, model outputs, gateway payloads, or credentials.",
            ),
            ReleaseCheck(
                ...
                validation_command="python -m pytest tests/test_legal_rag_embedding_index_commit_review_packet.py tests/test_legal_rag_embedding_batch_observation_gate.py tests/test_release_readiness.py tests/test_continuous_update_ledger.py tests/test_maintenance_evidence.py tests/test_frontend_ui_regression_gate.py -q && cd ../frontend && npm run typecheck && npm run ui:regression",
                manual_note="This is metadata-only Legal RAG embedding index commit review packet evidence for reviewing ready aggregate embedding observations before any real index commit, vector-slot matches, observed chunk/cost evidence, required maintainer/RAG-index/privacy signoffs, pre-commit checks, and prepare/hold/block commit-review actions; it does not claim maintainer commit approval, execute embeddings, call NewAPI, Gemini, models, gateways, or the network, write indexes or databases, write commit records, download datasets, collect committer identity, store or return source ids, approval item ids, raw query, raw retrieved context, raw legal text, source chunks, embedding vectors, prompts, model outputs, gateway payloads, or credentials.",
            ),
            ReleaseCheck(
                ...
                validation_command="python -m pytest tests/test_legal_rag_embedding_index_post_commit_verification_gate.py tests/test_legal_rag_embedding_index_commit_review_packet.py tests/test_release_readiness.py tests/test_continuous_update_ledger.py tests/test_maintenance_evidence.py tests/test_frontend_ui_regression_gate.py -q && cd ../frontend && npm run typecheck && npm run ui:regression",
                manual_note="This is metadata-only Legal RAG embedding index post-commit verification gate evidence for reviewing post-commit index observations after commit review, expected versus observed vector slots, index entry counts, metadata records, retrieval locators, checksum records, failed-entry totals, rollback signals, and allow/hold/block retrieval-diagnostics review actions; it does not claim maintainer commit approval, execute embeddings, call NewAPI, Gemini, models, gateways, or the network, write indexes or databases, write commit records, enable production retrieval, download datasets, collect committer identity, store or return source ids, approval item ids, raw query, raw retrieved context, raw legal text, source chunks, embedding vectors, prompts, model outputs, gateway payloads, or credentials.",
            ),
            ReleaseCheck(
                ...
                validation_command="python -m pytest tests/test_legal_rag_embedding_retrieval_diagnostics_handoff_gate.py tests/test_legal_rag_embedding_index_post_commit_verification_gate.py tests/test_release_readiness.py tests/test_continuous_update_ledger.py tests/test_maintenance_evidence.py tests/test_frontend_ui_regression_gate.py -q && cd ../frontend && npm run typecheck && npm run ui:regression",
                manual_note="This is metadata-only Legal RAG embedding retrieval diagnostics handoff gate evidence for reviewing safe handoff rows between post-commit verification and retrieval diagnostics, ready/hold/block handoff statuses, safe handoff payload fields, rollback review links, and retrieval-diagnostics-review-only actions; it does not execute retrieval diagnostics, enable production retrieval, claim index or retrieval quality, execute embeddings, call NewAPI, Gemini, models, gateways, or the network, write indexes or databases, write commit records, download datasets, collect committer identity, store or return source ids, raw query, user questions, retrieved context, raw legal text, source chunks, embedding vectors, prompts, model outputs, gateway payloads, or credentials.",
            ),
            ReleaseCheck(
                ...
                validation_command="python -m pytest tests/test_legal_rag_benchmark_alignment.py tests/test_legal_rag_retrieval_diagnostics_gate.py tests/test_legal_benchmark_fixture_crosswalk.py tests/test_release_readiness.py tests/test_continuous_update_ledger.py tests/test_frontend_ui_regression_gate.py -q && cd ../frontend && npm run typecheck && npm run ui:regression",
                manual_note="This is metadata-only Legal RAG benchmark alignment evidence for mapping LegalBench-RAG, CRAG, RAGAS, and Legal RAG Bench signals to local retrieval diagnostics, abstention escalation, public benchmark sampler, fixture crosswalk, and cheap-first Gemini/NewAPI default boundaries; it does not call NewAPI, Gemini, models, gateways, or the network, download public datasets, return public benchmark text, raw query, raw retrieved context, raw legal text, prompts, model outputs, or credentials.",
            ),
            ReleaseCheck(
                ...
                validation_command="python -m pytest tests/test_legal_rag_retrieval_observation_gate.py tests/test_legal_rag_selected_source_validation.py tests/test_release_readiness.py tests/test_continuous_update_ledger.py tests/test_maintenance_evidence.py -q && cd ../frontend && npm run typecheck && npm run ui:regression",
                manual_note="This is metadata-only Legal RAG retrieval observation gate evidence for sanitized local retrieval observations, selected-source citation validation counts, top-k depth, jurisdiction/freshness, source coverage, cheap-first routing decisions, release action labels, typed maintenance API helpers, and maintenance UI review; it does not call NewAPI, Gemini, models, gateways, or the network, download datasets, store or return source ids, raw query, raw retrieved context, raw legal text, prompts, model outputs, or credentials.",
            ),
            ReleaseCheck(
                ...
                validation_command="python -m pytest tests/test_legal_rag_answer_release_readiness_gate.py tests/test_legal_rag_retrieval_observation_gate.py tests/test_release_readiness.py tests/test_continuous_update_ledger.py tests/test_maintenance_evidence.py tests/test_frontend_ui_regression_gate.py -q && cd ../frontend && npm run typecheck && npm run ui:regression",
                manual_note="This is metadata-only Legal RAG answer release readiness gate evidence for converting sanitized retrieval observation rows into ready/review/block answer-release rows, internal answer draft actions, citation packet requirements, lawyer-review requirements, cheap-first verify/escalate boundaries, and client-delivery false flags; it does not call NewAPI, Gemini, models, gateways, or the network, write answers, send client delivery, claim legal advice or answer quality, download datasets, store or return source ids, raw query, user questions, raw retrieved context, raw legal text, prompts, model outputs, gateway payloads, or credentials.",
            ),
            ReleaseCheck(
