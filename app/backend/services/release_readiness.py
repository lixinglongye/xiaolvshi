    ...
    def _checks(self) -> list[ReleaseCheck]:
        return [
            ...
            ReleaseCheck(
                ...
                validation_command="python -m pytest tests/test_model_ops_newapi_channel_bootstrap.py tests/test_model_gateway_connection_profile.py tests/test_model_gateway_runtime_configuration.py tests/test_model_ops_observed_gemini_premium_exception_review.py tests/test_release_readiness.py tests/test_continuous_update_ledger.py tests/test_maintenance_evidence.py tests/test_frontend_ui_regression_gate.py -q",
                manual_note="This is metadata-only NewAPI channel cheap-first bootstrap evidence for yibuapi/OpenAI-compatible channel URL normalization, APP_AI_KEY placeholder setup, cheap-first Gemini defaults, sanitized observed model intake, coverage-gap review, and explicit-only premium exception review; it does not call NewAPI, Gemini, OpenAI, Google, yibuapi, gateways, app AI endpoints, models, or the network, does not write environment files, configuration, default routes, or traffic, and does not include API keys, Authorization headers, request bodies, response bodies, prompts, raw payloads, legal text, model outputs, gateway responses, credentials, emails, or user identifiers.",
            ),
            ReleaseCheck(
