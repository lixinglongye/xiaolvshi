...
class ModelConfigurationAuditService:
    ...
    def _configured_roles(self) -> list[ConfiguredModelRole]:
        ...
        return [
            ...
            ConfiguredModelRole(
                id="grounded-research-route-model",
                label="Grounded research task default",
                model=task_default_model("grounded-research"),
                env_var="APP_AI_GROUNDED_RESEARCH_MODEL",
                max_cost_tier="low",
                preferred_cost_tier="low",
                required_capabilities=("text", "grounding"),
                preferred_capabilities=("json", "agentic"),
                rationale="Grounded legal research should start on a low-cost Gemini model with grounding support.",
            ),
            ConfiguredModelRole(
                id="agentic-route-model",
                label="Agentic task default",
                model=task_default_model("agentic"),
                env_var="APP_AI_AGENTIC_MODEL",
                max_cost_tier="low",
                preferred_cost_tier="low",
                required_capabilities=("text", "agentic"),
                preferred_capabilities=("json", "grounding"),
                rationale="Agentic planning should use a low-cost Gemini model with tool/workflow capability before escalation.",
            ),
            ConfiguredModelRole(
