...
class ModelGatewayHealthPlanService:
    ...
    def build_plan(self) -> dict[str, Any]:
        ...
        return {
            ...
            "summary": {
                ...
                "cheap_first_low_cost_count": sum(1 for row in role_rows if row["role"] in LOW_RESOURCE_TASKS and row["cheap_first_aligned"]),
            ...
            "dry_run_contracts": self._dry_run_contracts(normalized_base_url, cheap_model),
            "checks": checks,
            ...
            rows.append(
                {
                    ...
                    "estimated_probe_cost_usd": estimate_token_cost_usd(model, PROBE_PROMPT_TOKENS, PROBE_COMPLETION_TOKENS),
    ...
    def _role_reason(self, role: str, known: bool, cost_tier: str | None, cheap_first_aligned: bool) -> str:
        ...
        if role in LOW_RESOURCE_TASKS and not cheap_first_aligned:
    ...
    def _checks(
        ...
        missing_low_cost = [
            ...
            if row["role"] in LOW_RESOURCE_TASKS and not row["cheap_first_aligned"] and row["is_known_model"]
        ]
        ...
        return [
            ...
            },
            {
                ...
                "reason": "High-volume roles use known low-cost Gemini models."
                if not missing_low_cost and not unknown_roles
                else (
                    f"High-volume roles drifted from low-cost models: {', '.join(missing_low_cost)}."
                    if missing_low_cost
    ...
    def _dry_run_contracts(self, normalized_base_url: str | None, cheap_model: str) -> list[dict[str, Any]]:
