...
async def dev_login(
    ...
    This endpoint is intentionally limited to localhost unless ENABLE_DEV_LOGIN=true
    is explicitly set. It is for local testing only.
    """
    if not is_local_dev_request(request):
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Dev login is only available locally")

    user_id = f"dev-{role}"
    user_email = f"{role}@local.test"
    user_name = "本地测试管理员" if role == "admin" else "本地测试用户"
    ...
    if user:
        ...
        user = User(id=user_id, email=user_email, name=user_name, role=role)
        db.add(user)
    ...
    auth_service = AuthService(db)
    app_token, expires_at, claims = await auth_service.issue_app_token(user=user)
