...
async def list_models():
    ...
    aihub_media_runtime_compatibility_gate = ModelOpsAIHubMediaRuntimeCompatibilityGateService().build_gate()
    ...
    model_ops_signals = {
        ...
        "aihub_media_runtime_compatibility_gate": aihub_media_runtime_compatibility_gate,
    ...
    legal_benchmark_default_promotion_checklist = (
        ModelOpsLegalBenchmarkDefaultPromotionChecklistService().build_checklist(model_ops_signals)
    )
    model_ops_signals["legal_benchmark_default_promotion_checklist"] = legal_benchmark_default_promotion_checklist
    legal_benchmark_default_promotion_signoff_packet = (
        ModelOpsLegalBenchmarkDefaultPromotionSignoffPacketService().build_packet(model_ops_signals)
    )
    model_ops_signals["legal_benchmark_default_promotion_signoff_packet"] = (
        legal_benchmark_default_promotion_signoff_packet
    )
    legal_benchmark_default_promotion_execution_handoff = (
        ModelOpsLegalBenchmarkDefaultPromotionExecutionHandoffService().build_handoff(model_ops_signals)
    )
...
@router.get("/models/gateway-runtime-configuration")
async def model_gateway_runtime_configuration():
    """Return safe OpenAI-compatible runtime gateway configuration evidence."""
    models_payload = await list_models()
    return {
        "success": True,
        "data": models_payload["gateway_runtime_configuration"],
    }


@router.post("/models/gateway-runtime-configuration")
async def evaluate_model_gateway_runtime_configuration(payload: dict[str, Any]):
    """Evaluate sanitized runtime gateway configuration metadata without network calls."""
    return {
        "success": True,
...
@router.get("/models/aihub-media-runtime-compatibility-gate")
async def modelops_aihub_media_runtime_compatibility_gate():
    """Return metadata-only AIHub media runtime compatibility evidence."""
    models_payload = await list_models()
    return {
        "success": True,
        "data": models_payload["aihub_media_runtime_compatibility_gate"],
    }


@router.post("/models/aihub-media-runtime-compatibility-gate")
async def evaluate_modelops_aihub_media_runtime_compatibility_gate(payload: dict[str, Any]):
    """Evaluate metadata-only AIHub media runtime compatibility without provider calls."""
    return {
        "success": True,
        "data": ModelOpsAIHubMediaRuntimeCompatibilityGateService().build_gate(payload),
    }
...
@router.get("/models/legal-benchmark-default-promotion-checklist")
async def model_ops_legal_benchmark_default_promotion_checklist():
    """Return legal benchmark default-promotion maintainer checklist evidence."""
    models_payload = await list_models()
    return {
        "success": True,
        "data": models_payload["legal_benchmark_default_promotion_checklist"],
    }


@router.post("/models/legal-benchmark-default-promotion-checklist")
async def evaluate_model_ops_legal_benchmark_default_promotion_checklist(payload: dict[str, Any]):
    """Evaluate legal benchmark default-promotion checklist without model or gateway calls."""
    return {
        "success": True,
        "data": ModelOpsLegalBenchmarkDefaultPromotionChecklistService().build_checklist(payload),
    }


@router.get("/models/legal-benchmark-default-promotion-signoff-packet")
async def model_ops_legal_benchmark_default_promotion_signoff_packet():
    """Return legal benchmark default-promotion signoff packet evidence."""
    models_payload = await list_models()
    return {
        "success": True,
        "data": models_payload["legal_benchmark_default_promotion_signoff_packet"],
    }


@router.post("/models/legal-benchmark-default-promotion-signoff-packet")
async def evaluate_model_ops_legal_benchmark_default_promotion_signoff_packet(payload: dict[str, Any]):
    """Evaluate legal benchmark default-promotion signoff packet without model or gateway calls."""
    return {
        "success": True,
        "data": ModelOpsLegalBenchmarkDefaultPromotionSignoffPacketService().build_packet(payload),
    }


@router.get("/models/legal-benchmark-default-promotion-execution-handoff")
async def model_ops_legal_benchmark_default_promotion_execution_handoff():
    """Return legal benchmark default-promotion execution handoff and rollback gate evidence."""
    models_payload = await list_models()
    return {
        "success": True,
        "data": models_payload["legal_benchmark_default_promotion_execution_handoff"],
    }


@router.post("/models/legal-benchmark-default-promotion-execution-handoff")
async def evaluate_model_ops_legal_benchmark_default_promotion_execution_handoff(payload: dict[str, Any]):
    """Evaluate legal benchmark default-promotion execution handoff without model or gateway calls."""
    return {
        "success": True,
