...
@router.put("/local-upload", response_model=ObjectInfo)
async def local_upload_file(
    ...
    """Local development upload target used when OSS is not configured."""
    try:
        request = FileUpDownRequest(bucket_name=bucket_name, object_key=object_key)
        data = await raw_request.body()
        service = StorageService()
        return await service.save_local_upload(request, token=token, data=data)
    except ValueError as e:
        logger.error(f"Invalid local upload request: {e}")
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))
    except Exception as e:
        logger.error(f"Failed local upload: {e}", exc_info=True)
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"{e}")
...
async def local_download_file(
    ...
    """Local development download target used when OSS is not configured."""
    try:
        request = FileUpDownRequest(bucket_name=bucket_name, object_key=object_key)
        service = StorageService()
        path = service.resolve_local_download(request, token=token)
        return FileResponse(path, filename=path.name)
    except ValueError as e:
        logger.error(f"Invalid local download request: {e}")
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))
    except Exception as e:
        logger.error(f"Failed local download: {e}", exc_info=True)
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"{e}")
