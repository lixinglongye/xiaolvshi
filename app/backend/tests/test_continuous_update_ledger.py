...
def test_continuous_update_ledger_includes_modelops_legal_benchmark_default_promotion_signoff_packet():
    ledger = ContinuousUpdateLedgerService().build_ledger()
    entry = next(
        item for item in ledger["completed_updates"]
        if item["id"] == "modelops-legal-benchmark-default-promotion-signoff-packet"
    )

    assert entry["category"] == "model_ops"
    assert entry["size"] == "medium"
    assert "metadata-only legal benchmark default-promotion signoff packet evidence" in entry["impact"]
    assert "checklist rows" in entry["impact"]
    assert "approver identity collection" in entry["impact"]
    assert "gateway/network calls" in entry["impact"]
    assert "raw legal text" in entry["impact"]
    assert "app/backend/services/modelops_legal_benchmark_default_promotion_signoff_packet.py" in entry["evidence_paths"]
    assert "app/backend/tests/test_modelops_legal_benchmark_default_promotion_signoff_packet.py" in entry["evidence_paths"]
    assert "app/backend/services/modelops_legal_benchmark_default_promotion_checklist.py" in entry["evidence_paths"]
    assert "app/frontend/src/pages/ModelOpsPage.tsx" in entry["evidence_paths"]
    assert "docs/MODELOPS_LEGAL_BENCHMARK_DEFAULT_PROMOTION_SIGNOFF_PACKET.md" in entry["evidence_paths"]
    assert "modelops-legal-benchmark-default-promotion-signoff-packet" in entry["release_gate_links"]
    assert "modelops-legal-benchmark-default-promotion-checklist" in entry["release_gate_links"]
    assert "model-ops-default-change-queue" in entry["release_gate_links"]
    assert "low-cost-routing" in entry["user_need_ids"]
    assert any(
        "tests/test_modelops_legal_benchmark_default_promotion_signoff_packet.py" in command
        for command in ledger["validation_commands"]
    )


def test_continuous_update_ledger_includes_modelops_legal_benchmark_default_promotion_execution_handoff():
    ledger = ContinuousUpdateLedgerService().build_ledger()
    entry = next(
        item for item in ledger["completed_updates"]
        if item["id"] == "modelops-legal-benchmark-default-promotion-execution-handoff"
    )

    assert entry["category"] == "model_ops"
    assert entry["size"] == "medium"
    assert "metadata-only legal benchmark default-promotion execution handoff" in entry["impact"]
    assert "rollback gate evidence" in entry["impact"]
    ...
    assert "app/backend/tests/test_modelops_legal_benchmark_default_promotion_execution_handoff.py" in entry["evidence_paths"]
    assert "app/backend/services/modelops_legal_benchmark_default_promotion_signoff_packet.py" in entry["evidence_paths"]
    assert "app/frontend/src/pages/ModelOpsPage.tsx" in entry["evidence_paths"]
    assert "docs/MODELOPS_LEGAL_BENCHMARK_DEFAULT_PROMOTION_EXECUTION_HANDOFF.md" in entry["evidence_paths"]
    assert "modelops-legal-benchmark-default-promotion-execution-handoff" in entry["release_gate_links"]
    assert "modelops-legal-benchmark-default-promotion-signoff-packet" in entry["release_gate_links"]
    assert "model-ops-default-change-queue" in entry["release_gate_links"]
    assert "low-cost-routing" in entry["user_need_ids"]
    assert any(
        "tests/test_modelops_legal_benchmark_default_promotion_execution_handoff.py" in command
        for command in ledger["validation_commands"]
    )
...
def test_continuous_update_ledger_prioritizes_low_resource_next_work():
    ...
    assert "route-telemetry-result-archive" in completed_ids
    ...
    assert "route-telemetry-result-archive" not in queue_ids
    ...
    embedding_preflight_entry = next(
        entry
        for entry in ledger["completed_updates"]
        if entry["id"] == "legal-rag-embedding-batch-preflight"
    )
    assert embedding_preflight_entry["size"] == "medium"
    assert embedding_preflight_entry["status"] == "shipped"
    assert embedding_preflight_entry["category"] == "benchmark"
    assert "POST /api/v1/legal-rag/embedding-batch-preflight" in embedding_preflight_entry["impact"]
    assert "LegalRagEmbeddingBatchPreflightService" in embedding_preflight_entry["impact"]
    assert "hashes chunk ids and text" in embedding_preflight_entry["impact"]
    assert "cheap-first Gemini embedding tokens" in embedding_preflight_entry["impact"]
    assert "duplicate chunks" in embedding_preflight_entry["impact"]
    assert "PII signals" in embedding_preflight_entry["impact"]
    assert "secret-like inputs" in embedding_preflight_entry["impact"]
    assert "without NewAPI/Gemini/model/gateway/network calls" in embedding_preflight_entry["impact"]
    assert "source text" in embedding_preflight_entry["impact"]
    assert "embedding vectors" in embedding_preflight_entry["impact"]
    assert "credentials" in embedding_preflight_entry["impact"]
    assert "app/backend/services/legal_rag_embedding_batch_preflight.py" in embedding_preflight_entry["evidence_paths"]
    assert "app/backend/tests/test_legal_rag_embedding_batch_preflight.py" in embedding_preflight_entry["evidence_paths"]
    assert "app/backend/routers/legal_rag.py" in embedding_preflight_entry["evidence_paths"]
    assert "app/backend/routers/maintenance.py" in embedding_preflight_entry["evidence_paths"]
    assert "docs/LEGAL_RAG_EMBEDDING_BATCH_PREFLIGHT.md" in embedding_preflight_entry["evidence_paths"]
    ...
    assert "maintenance evidence panel" in newapi_channel_bootstrap_entry["impact"]
    ...
    assert "app/backend/routers/maintenance.py" in newapi_channel_bootstrap_entry["evidence_paths"]
    assert "app/frontend/src/lib/modelOpsApi.ts" in newapi_channel_bootstrap_entry["evidence_paths"]
    assert "app/frontend/src/lib/maintenanceApi.ts" in newapi_channel_bootstrap_entry["evidence_paths"]
    assert "app/frontend/src/pages/ModelOpsPage.tsx" in newapi_channel_bootstrap_entry["evidence_paths"]
    assert "app/frontend/src/pages/MaintenanceEvidencePage.tsx" in newapi_channel_bootstrap_entry["evidence_paths"]
    ...
    result_archive_entry = next(
        entry for entry in ledger["completed_updates"] if entry["id"] == "route-telemetry-result-archive"
    )
    assert result_archive_entry["category"] == "model_ops"
    assert result_archive_entry["size"] == "medium"
    assert result_archive_entry["status"] == "shipped"
    assert "metadata-only route telemetry result archive" in result_archive_entry["impact"]
    assert "cost ledger evidence" in result_archive_entry["impact"]
    assert "daily archive rows" in result_archive_entry["impact"]
    assert "task/model cost ledger rows" in result_archive_entry["impact"]
    assert "triage/remediation release-review links" in result_archive_entry["impact"]
    assert "cheap-first review boundaries" in result_archive_entry["impact"]
    assert "model calls" in result_archive_entry["impact"]
    assert "gateway calls" in result_archive_entry["impact"]
    assert "configuration writes" in result_archive_entry["impact"]
    assert "default route changes" in result_archive_entry["impact"]
    assert "request bodies" in result_archive_entry["impact"]
    assert "response bodies" in result_archive_entry["impact"]
    assert "headers" in result_archive_entry["impact"]
    assert "gateway responses" in result_archive_entry["impact"]
    assert "credentials" in result_archive_entry["impact"]
    assert "app/backend/services/route_telemetry_result_archive.py" in result_archive_entry["evidence_paths"]
    assert "app/backend/tests/test_route_telemetry_result_archive.py" in result_archive_entry["evidence_paths"]
    assert "app/backend/services/model_ops_readiness.py" in result_archive_entry["evidence_paths"]
    assert "app/backend/services/release_readiness.py" in result_archive_entry["evidence_paths"]
    assert "app/backend/services/frontend_ui_regression_gate.py" in result_archive_entry["evidence_paths"]
    assert "app/frontend/src/lib/modelOpsApi.ts" in result_archive_entry["evidence_paths"]
    assert "app/frontend/src/pages/ModelOpsPage.tsx" in result_archive_entry["evidence_paths"]
    assert "app/frontend/scripts/ui-regression.mjs" in result_archive_entry["evidence_paths"]
    assert "docs/ROUTE_TELEMETRY_RESULT_ARCHIVE.md" in result_archive_entry["evidence_paths"]
    assert "docs/CONTINUOUS_UPDATE_LEDGER.md" in result_archive_entry["evidence_paths"]
    assert "route-telemetry-result-archive" in result_archive_entry["release_gate_links"]
    assert "route-telemetry-repository" in result_archive_entry["release_gate_links"]
    assert "route-telemetry-ops-summary" in result_archive_entry["release_gate_links"]
    assert "route-telemetry-triage-queue" in result_archive_entry["release_gate_links"]
    assert "route-telemetry-remediation-plan" in result_archive_entry["release_gate_links"]
    assert "model-ops-readiness" in result_archive_entry["release_gate_links"]
    assert "frontend-ui-regression" in result_archive_entry["release_gate_links"]
    assert (
        "python -m pytest tests/test_route_telemetry_result_archive.py tests/test_release_readiness.py "
        "tests/test_continuous_update_ledger.py tests/test_frontend_ui_regression_gate.py -q "
        "&& cd ../frontend && npm run typecheck && npm run ui:regression"
        in ledger["validation_commands"]
    )
    ...
    assert "direct AIHub route" in route_coverage_entry["impact"]
    assert "ModelOps page" in route_coverage_entry["impact"]
    ...
    assert "app/backend/routers/aihub.py" in route_coverage_entry["evidence_paths"]
    assert "app/backend/routers/maintenance.py" in route_coverage_entry["evidence_paths"]
    assert "app/frontend/src/lib/modelOpsApi.ts" in route_coverage_entry["evidence_paths"]
    assert "app/frontend/src/lib/maintenanceApi.ts" in route_coverage_entry["evidence_paths"]
    assert "app/frontend/src/pages/ModelOpsPage.tsx" in route_coverage_entry["evidence_paths"]
