...
def test_legal_document_benchmark_coverage_tracks_all_target_document_types():
    matrix = LegalDocumentBenchmarkCoverageService().build_matrix()
    ...
    assert set(document_type_rows) == set(TARGET_DOCUMENT_TYPES)
    assert all(document_type_rows[document_type]["covered"] is True for document_type in TARGET_DOCUMENT_TYPES)
    assert all(document_type_rows[document_type]["coverage_count"] == 1 for document_type in TARGET_DOCUMENT_TYPES)
...
def test_legal_document_benchmark_coverage_queue_is_laptop_safe():
    ...
    assert len(queue) <= MAX_LOCAL_FIXTURES_PER_RUN
    assert queue == []
    assert any("fixture matrix" in action for action in matrix["recommended_actions"])
    assert any("benchmark suite" in action for action in matrix["recommended_actions"])
