...
def test_maintenance_profile_links_reviewable_evidence():
    ...
    assert "Legal document benchmark route plan replay" in profile["release_management"]["release_readiness_controls"]
    assert (
    ...
    assert "Legal RAG answer release readiness gate" in profile["release_management"]["release_readiness_controls"]
    ...
    assert "app/backend/services/legal_document_benchmark_route_plan.py" in evidence_paths
    ...
    assert "app/backend/services/legal_document_benchmark_route_plan_execution_result_handoff.py" in evidence_paths
    assert "app/backend/services/legal_document_benchmark_route_plan_execution_review_packet.py" in evidence_paths
    ...
    assert "app/backend/tests/test_legal_document_benchmark_route_plan.py" in evidence_paths
    ...
    assert "app/backend/tests/test_legal_document_benchmark_route_plan_execution_result_handoff.py" in evidence_paths
    assert "app/backend/tests/test_legal_document_benchmark_route_plan_execution_review_packet.py" in evidence_paths
    ...
    assert "docs/LEGAL_DOCUMENT_BENCHMARK_ROUTE_PLAN.md" in evidence_paths
    ...
    assert "docs/LEGAL_DOCUMENT_BENCHMARK_ROUTE_PLAN_EXECUTION_RESULT_HANDOFF.md" in evidence_paths
    assert "docs/LEGAL_DOCUMENT_BENCHMARK_ROUTE_PLAN_EXECUTION_REVIEW_PACKET.md" in evidence_paths
    ...
    assert "app/backend/services/feedback_user_need_legal_document_benchmark_backlog.py" in evidence_paths
    assert "app/backend/tests/test_feedback_user_need_legal_document_benchmark_backlog.py" in evidence_paths
    assert "docs/FEEDBACK_USER_NEED_LEGAL_DOCUMENT_BENCHMARK_BACKLOG.md" in evidence_paths
    ...
    assert "metadata-only legal document route-plan replay, research-alignment, execution-readiness, execution-result archive, release-evidence handoff, and execution-review packet evidence" in quality_signal[
        "description"
    ]
    ...
    assert "route-plan replay, research-alignment, execution-readiness, execution-result archive, handoff, and review-packet review" in quality_signal[
        "responsibility"
    ]
    ...
    assert "app/backend/services/feedback_user_need_legal_document_benchmark_backlog.py" in quality_signal["evidence_paths"]
    assert "app/backend/tests/test_feedback_user_need_legal_document_benchmark_backlog.py" in quality_signal["evidence_paths"]
    assert "docs/FEEDBACK_USER_NEED_LEGAL_DOCUMENT_BENCHMARK_BACKLOG.md" in quality_signal["evidence_paths"]
    ...
    assert any("without writing indexes or databases" in guardrail for guardrail in profile["application_guardrails"])
    ...
    assert any("Legal RAG answer release readiness gate is metadata-only answer-release evidence" in guardrail for guardrail in profile["application_guardrails"])
    assert any("client-delivery false flags" in guardrail for guardrail in profile["application_guardrails"])
    assert any("without calling NewAPI" in guardrail for guardrail in profile["application_guardrails"])
    assert any("claiming legal advice or answer quality" in guardrail for guardrail in profile["application_guardrails"])
    ...
    assert any(
        "legal document benchmark route-plan replay is metadata-only route evidence" in guardrail
        and "unapproved premium route-down" in guardrail
        and "scenario payloads" in guardrail
        for guardrail in profile["application_guardrails"]
    )
    assert any(
        "legal document benchmark route-plan research alignment is metadata-only source alignment evidence"
        in guardrail
        and "FrugalGPT" in guardrail
        and "LegalBench-RAG" in guardrail
        and "LexEval" in guardrail
        and "public benchmark text" in guardrail
        for guardrail in profile["application_guardrails"]
    )
    assert any(
        "legal document benchmark route-plan execution readiness packet is metadata-only pre-run evidence"
        in guardrail
    ...
    assert any(
        "legal document benchmark route-plan execution result handoff is metadata-only release-evidence handoff evidence"
        in guardrail
        and "fixture_limit=3" in guardrail
        and "max_parallel_model_requests=1" in guardrail
        and "write release records" in guardrail
        and "record maintainer approval" in guardrail
        and "credentials" in guardrail
        for guardrail in profile["application_guardrails"]
    )
    assert any(
        "legal document benchmark route-plan execution review packet is metadata-only reviewer evidence"
        in guardrail
        and "fixture_limit=3" in guardrail
        and "max_parallel_model_requests=1" in guardrail
        and "write release records" in guardrail
