...
def test_modelops_legal_benchmark_default_promotion_observation_gate_is_required_model_ops_gate():
    service = ReleaseReadinessService()
    commands = [
        item for item in service.default_validation_commands()
        if item["check_id"] == "modelops-legal-benchmark-default-promotion-observation-gate"
    ]
    result = service.evaluate({"modelops-legal-benchmark-default-promotion-observation-gate": "not_run"})
    check = next(
        check
        for check in result["checks"]
        if check["id"] == "modelops-legal-benchmark-default-promotion-observation-gate"
    )

    assert commands == [
        {
            "check_id": "modelops-legal-benchmark-default-promotion-observation-gate",
            "command": (
                "python -m pytest tests/test_modelops_legal_benchmark_default_promotion_observation_gate.py "
                "tests/test_modelops_legal_benchmark_default_promotion_execution_handoff.py tests/test_model_ops_readiness.py "
                "tests/test_release_readiness.py tests/test_continuous_update_ledger.py "
                "tests/test_frontend_ui_regression_gate.py -q && cd ../frontend && "
                "npm run typecheck && npm run ui:regression"
            ),
    ...
    assert check["required"] is True
    assert check["blocks_release"] is True
    assert "metadata-only legal benchmark default-promotion post-execution observation gate" in check["manual_note"]
    assert "rollback-window" in check["manual_note"]
    assert "execute rollback" in check["manual_note"]
    assert "does not call NewAPI" in check["manual_note"]
    assert "raw legal text" in check["manual_note"]
    assert "app/backend/services/modelops_legal_benchmark_default_promotion_observation_gate.py" in check["evidence_paths"]
    assert "app/backend/tests/test_modelops_legal_benchmark_default_promotion_observation_gate.py" in check["evidence_paths"]
    assert "app/backend/services/modelops_legal_benchmark_default_promotion_execution_handoff.py" in check["evidence_paths"]
    assert "app/frontend/src/pages/ModelOpsPage.tsx" in check["evidence_paths"]
    assert "docs/MODELOPS_LEGAL_BENCHMARK_DEFAULT_PROMOTION_OBSERVATION_GATE.md" in check["evidence_paths"]
...
def test_recent_backend_product_slices_are_optional_release_evidence():
    ...
    expected_commands = {
        ...
        "legal-document-benchmark-release-scorecard": "python -m pytest tests/test_legal_document_benchmark_release_scorecard.py tests/test_legal_document_benchmark_suite.py tests/test_legal_document_benchmark_coverage.py tests/test_legal_document_fact_consistency_benchmark.py tests/test_legal_document_benchmark_route_plan.py tests/test_legal_document_coverage_claim_policy.py tests/test_legal_document_benchmark_route_plan_execution_claim_gate.py tests/test_release_readiness.py tests/test_continuous_update_ledger.py tests/test_maintenance_evidence.py tests/test_frontend_ui_regression_gate.py -q && cd ../frontend && npm run typecheck && npm run ui:regression",
    ...
    assert "cheap-first Gemini route plan" in checks["legal-document-benchmark-route-plan"]["manual_note"]
    assert "Flash-Lite prechecks" in checks["legal-document-benchmark-route-plan"]["manual_note"]
    assert "premium-default blocking" in checks["legal-document-benchmark-route-plan"]["manual_note"]
    assert "does not call NewAPI" in checks["legal-document-benchmark-route-plan"]["manual_note"]
    assert "raw fixture snippets" in checks["legal-document-benchmark-route-plan"]["manual_note"]
    assert "credentials" in checks["legal-document-benchmark-route-plan"]["manual_note"]
    assert "app/backend/services/legal_document_benchmark_route_plan.py" in checks[
        "legal-document-benchmark-route-plan"
    ]["evidence_paths"]
    assert "app/backend/tests/test_legal_document_benchmark_route_plan.py" in checks[
        "legal-document-benchmark-route-plan"
    ]["evidence_paths"]
    assert "app/backend/routers/maintenance.py" in checks["legal-document-benchmark-route-plan"]["evidence_paths"]
    ...
    assert "execution readiness packet" in checks[
        "legal-document-benchmark-route-plan-execution-readiness"
    ]["manual_note"]
    assert "fixture_limit=3" in checks["legal-document-benchmark-route-plan-execution-readiness"]["manual_note"]
    assert "max_parallel_model_requests=1" in checks[
        "legal-document-benchmark-route-plan-execution-readiness"
    ]["manual_note"]
    assert "does not call NewAPI" in checks["legal-document-benchmark-route-plan-execution-readiness"][
        "manual_note"
    ]
    assert "record maintainer approval" in checks["legal-document-benchmark-route-plan-execution-readiness"][
        "manual_note"
    ]
    assert "scenario payloads" in checks["legal-document-benchmark-route-plan-execution-readiness"][
        "manual_note"
    ]
    assert "credentials" in checks["legal-document-benchmark-route-plan-execution-readiness"]["manual_note"]
    assert "app/backend/services/legal_document_benchmark_route_plan_execution_readiness.py" in checks[
        "legal-document-benchmark-route-plan-execution-readiness"
    ]["evidence_paths"]
    assert "app/backend/tests/test_legal_document_benchmark_route_plan_execution_readiness.py" in checks[
        "legal-document-benchmark-route-plan-execution-readiness"
    ]["evidence_paths"]
    assert "docs/LEGAL_DOCUMENT_BENCHMARK_ROUTE_PLAN_EXECUTION_READINESS.md" in checks[
        "legal-document-benchmark-route-plan-execution-readiness"
    ]["evidence_paths"]
    assert "execution result archive" in checks[
        "legal-document-benchmark-route-plan-execution-result-archive"
    ]["manual_note"]
    assert "case_id, phase, observed model, status, token, cost, latency" in checks[
        "legal-document-benchmark-route-plan-execution-result-archive"
    ]["manual_note"]
    assert "fixture_limit=3" in checks[
        "legal-document-benchmark-route-plan-execution-result-archive"
    ]["manual_note"]
    assert "max_parallel_model_requests=1" in checks[
        "legal-document-benchmark-route-plan-execution-result-archive"
    ...
    assert "request bodies" in checks[
        "legal-document-benchmark-route-plan-execution-result-archive"
    ]["manual_note"]
    assert "credentials" in checks["legal-document-benchmark-route-plan-execution-result-archive"]["manual_note"]
    assert "app/backend/services/legal_document_benchmark_route_plan_execution_result_archive.py" in checks[
        "legal-document-benchmark-route-plan-execution-result-archive"
    ]["evidence_paths"]
    assert "app/backend/tests/test_legal_document_benchmark_route_plan_execution_result_archive.py" in checks[
        "legal-document-benchmark-route-plan-execution-result-archive"
    ]["evidence_paths"]
    ...
    assert "execution review packet" in checks[
        "legal-document-benchmark-route-plan-execution-review-packet"
    ]["manual_note"]
    assert "attach/review/hold decisions" in checks[
        "legal-document-benchmark-route-plan-execution-review-packet"
    ]["manual_note"]
    assert "fixture_limit=3" in checks["legal-document-benchmark-route-plan-execution-review-packet"][
        "manual_note"
    ]
    assert "max_parallel_model_requests=1" in checks[
        "legal-document-benchmark-route-plan-execution-review-packet"
    ...
    assert "credentials" in checks["legal-document-benchmark-route-plan-execution-review-packet"]["manual_note"]
    assert "app/backend/services/legal_document_benchmark_route_plan_execution_review_packet.py" in checks[
        "legal-document-benchmark-route-plan-execution-review-packet"
    ]["evidence_paths"]
    assert "app/backend/tests/test_legal_document_benchmark_route_plan_execution_review_packet.py" in checks[
        "legal-document-benchmark-route-plan-execution-review-packet"
    ]["evidence_paths"]
    ...
    assert "release scorecard" in checks["legal-document-benchmark-release-scorecard"]["manual_note"]
    assert "metadata-only" in checks["legal-document-benchmark-release-scorecard"]["manual_note"]
    assert "public benchmark scores" in checks["legal-document-benchmark-release-scorecard"]["manual_note"]
    assert "live provider execution" in checks["legal-document-benchmark-release-scorecard"]["manual_note"]
    assert "production legal quality" in checks["legal-document-benchmark-release-scorecard"]["manual_note"]
    assert "raw legal text" in checks["legal-document-benchmark-release-scorecard"]["manual_note"]
    assert "credentials" in checks["legal-document-benchmark-release-scorecard"]["manual_note"]
    assert "app/backend/services/legal_document_benchmark_release_scorecard.py" in checks[
        "legal-document-benchmark-release-scorecard"
    ]["evidence_paths"]
    assert "app/backend/tests/test_legal_document_benchmark_release_scorecard.py" in checks[
        "legal-document-benchmark-release-scorecard"
    ]["evidence_paths"]
    assert "app/backend/routers/maintenance.py" in checks["legal-document-benchmark-release-scorecard"][
        "evidence_paths"
    ]
    assert "docs/LEGAL_DOCUMENT_BENCHMARK_RELEASE_SCORECARD.md" in checks[
        "legal-document-benchmark-release-scorecard"
    ]["evidence_paths"]
