...
delivery package manifests, contract clause extraction schema, role permission
matrices, billing usage quotas, billing quota persistence planning, feedback
lifecycle policy, legal source ingestion metadata, Gemini/NewAPI cheap-first
...
python -m pytest tests/test_case_workbench_payload.py tests/test_document_delivery_package_manifest.py tests/test_case_role_permission_matrix.py tests/test_billing_usage_quota_policy.py tests/test_billing_quota_persistence_plan.py tests/test_feedback_lifecycle_policy.py -q
```
