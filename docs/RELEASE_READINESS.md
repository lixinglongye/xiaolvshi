...
Recent optional evidence: `legal-document-benchmark-coverage`, `legal-document-benchmark-route-plan`, `legal-document-benchmark-route-plan-replay`, `legal-document-benchmark-route-plan-override-ui`, `legal-document-benchmark-coverage-ui`, `legal-document-benchmark-fixture-ui`, `frontend-ui-regression-gate`, and `legal-document-coverage-claim-policy` expose the local legal-document fixture coverage matrix, cheap-first route plan, route-plan replay, route override preview, and fixture-suite evaluator in backend release checks, the maintenance UI, reviewer-facing UI regression gates, and a claim-review route. The route plan maps each synthetic benchmark case to Flash-Lite prechecks, budgeted primary routes, runtime-router decisions, candidate ladders, and premium-default blocking without executing model calls. The replay checks default cheap-first routes, unapproved premium route-down behavior, simulated approved premium blocking, and grounded legal-opinion routing without calling models or returning scenario text. The override UI lets maintainers preview case id, primary task, model id, and approval-mode changes while blocking credential-shaped model input. The fixture service carries readable zh-CN locale-quality metadata and mojibake regression checks. The UI fixture and route-plan panels deliberately do not render raw fixture snippets, prompts, generated text, or gateway responses, and the checks do not claim external benchmark scores, model-run accuracy, universal document support, or real client-document testing.
