...
Recent optional evidence: `legal-document-benchmark-coverage`, `legal-document-benchmark-route-plan`, `legal-document-benchmark-route-plan-replay`, `legal-document-benchmark-route-plan-research-alignment`, `legal-document-benchmark-route-plan-execution-readiness`, `legal-document-benchmark-route-plan-execution-result-archive`, `legal-document-benchmark-route-plan-execution-result-handoff`, `legal-document-benchmark-route-plan-execution-review-packet`, `legal-document-benchmark-route-plan-execution-claim-gate`, `legal-document-benchmark-release-scorecard`, `legal-document-benchmark-route-plan-override-ui`, `legal-document-benchmark-coverage-ui`, `legal-document-benchmark-fixture-ui`, `frontend-ui-regression-gate`, and `legal-document-coverage-claim-policy` expose the local legal-document fixture coverage matrix, cheap-first route plan, route-plan replay, research/source alignment, execution-readiness packet, sanitized execution-result archive, release-evidence handoff, execution-review packet, execution-claim gate, release scorecard, route override preview, and fixture-suite evaluator in backend release checks, the maintenance UI, reviewer-facing UI regression gates, and a claim-review route. The route plan maps each synthetic benchmark case to Flash-Lite prechecks, budgeted primary routes, runtime-router decisions, candidate ladders, and premium-default blocking without executing model calls. The replay checks default cheap-first routes, unapproved premium route-down behavior, simulated approved premium blocking, and grounded legal-opinion routing without calling models or returning scenario text. The research alignment maps replay scenarios to stored Gemini official model/pricing URLs, FrugalGPT, LegalBench-RAG, and LexEval anchors without downloading papers or public benchmark data. The execution-readiness packet joins route plan, replay, and research alignment into manual serial low-resource gates with `fixture_limit=3` and `max_parallel_model_requests=1` without running benchmarks, recording approval, or calling models. The execution-result archive accepts only sanitized case/model/status/token/cost/latency/fallback metadata from manual observations and does not write archive files, record approval, return request/response bodies, or claim benchmark execution. The execution-result handoff requires readiness/archive success and cheap-first alignment before sanitized observations can be attached as release evidence, and does not write release records, record approval, run benchmarks, call models, or claim public benchmark scores. The execution-review packet summarizes readiness, archive, and handoff status into attach/review/hold reviewer decisions without writing release records, recording approval, running benchmarks, calling models, or claiming public benchmark scores. The execution-claim gate hashes proposed release/support claims, allows only ready review-packet-backed metadata-only wording, and blocks public benchmark scores, live provider execution, approval, default changes, traffic shifts, production-quality claims, sensitive material, and raw claim text echo. The release scorecard aggregates the suite, coverage matrix, fact consistency suite, cheap-first route plan, coverage claim policy, and execution claim gate into ready/review/block components without calling models, running benchmarks by itself, writing release records, or claiming public scores, live provider execution, production legal quality, raw text retention, default changes, or traffic shifts. The override UI lets maintainers preview case id, primary task, model id, and approval-mode changes while blocking credential-shaped model input. The fixture service carries readable zh-CN locale-quality metadata and mojibake regression checks. The UI fixture and route-plan panels deliberately do not render raw fixture snippets, prompts, generated text, or gateway responses, and the checks do not claim external benchmark scores, model-run accuracy, universal document support, or real client-document testing.
...
```json
{
  ...
  "model-gateway-probe-runbook-gate": "pass",
...
The optional `legal-rag-embedding-batch-observation-gate` check records
metadata-only Legal RAG embedding batch observation evidence through
`LegalRagEmbeddingBatchObservationGateService` at
`/api/v1/maintenance/legal-rag-embedding-batch-observation-gate`. It reviews
sanitized aggregate observations after an external cheap Gemini embedding run:
observed batch/chunk/vector-slot/token counts, cost deltas,
`max_parallel_embedding_requests=1`, and allow/hold/block index-review actions.
It must not be used to claim maintainer approval, embedding execution by the
gate, vector storage, index/database writes, retrieval quality, legal-answer
quality, live pricing accuracy, provider execution, or client delivery. It must
not collect approver identity, call NewAPI, Gemini, models, gateways, app AI
endpoints, or the network, and it must not store or return source ids, approval
item ids, raw query, raw retrieved context, raw legal text, source chunks,
embedding vectors, prompts, model outputs, gateway payloads, credentials,
emails, or client material.
...
The optional `legal-rag-embedding-index-post-commit-verification-gate` check
records metadata-only Legal RAG embedding index post-commit verification
evidence through `LegalRagEmbeddingIndexPostCommitVerificationGateService` at
`/api/v1/maintenance/legal-rag-embedding-index-post-commit-verification-gate`.
It reviews sanitized post-commit index observations after commit review:
expected versus observed vector slots, index entry counts, metadata records,
retrieval locators, checksum records, failed-entry totals, rollback signals,
and allow/hold/block retrieval-diagnostics review actions. It must not be used
to claim maintainer commit approval, embedding execution, vector storage,
index/database writes, commit record writes, production retrieval enablement,
retrieval quality, legal-answer quality, live pricing accuracy, provider
execution, or client delivery. It must not collect committer identity, call
NewAPI, Gemini, models, gateways, app AI endpoints, or the network, and it must
not store or return source ids, approval item ids, raw query, raw retrieved
