...
- Model operations: model catalog, configuration audit, default optimization, model-ops readiness, budget policy, task inference, runtime routing, reasoning effort policy, request parameter policy, route telemetry, route guardrails, callsite audit, fallback chains, routing replay, usage-safe telemetry.
