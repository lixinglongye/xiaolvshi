...
```json
{
  "model_ops_readiness": {
    ...
    "summary": {
      "component_count": 66,
      "required_component_count": 65,
      "optional_component_count": 1,
      "pass_count": 41,
      "warn_count": 25,
      "fail_count": 0,
      "required_warning_count": 24,
      "optional_review_count": 1,
      "required_failure_count": 0,
      "optional_failure_count": 0,
      "blocking_count": 0,
      "warning_count": 25,
      "warning_drilldown_count": 25,
      "p0_warning_count": 0,
      "p1_warning_count": 24,
      "p2_warning_count": 1
    },
...
`legal-benchmark-default-promotion-bridge` is required metadata-only release
evidence for cheap-first legal default review. It joins legal fixture benchmark
gate, default-promotion packet, regression budget, evidence handoff, and Gemini
official lifecycle drift evidence into one review packet before a legal-task
default can be considered. It does not call models or gateways, write
...
`legal-benchmark-default-promotion-signoff-packet` is required metadata-only
release evidence for packaging checklist rows into external maintainer signoff
requirements. It returns signoff roles, pre-signoff checks, source status rows,
and release actions without calling models or gateways, writing configuration
or env files, recording approval or signoff, collecting approver identity,
shifting traffic, claiming maintainer approval, or returning raw legal text,
prompts, model output, payloads, emails, identifiers, or credentials.
